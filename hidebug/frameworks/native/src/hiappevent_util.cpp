...
{
    if (processId_ < 0 || (timeout_ == 0 && limitValue_ == 0)) {
        return;
    }
    ...
    if ((limitValue_ > 0 && records_.size() >= limitValue_) ||
        (timeout_ > 0 && lastReportTime_ + timeout_ * secondToNanosecond < GetElapsedNanoSecondsSinceBoot())) {
