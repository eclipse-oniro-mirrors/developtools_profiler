...
std::string GetLoopbackAddress()
{
    ...
    if (getifaddrs(&ifAddrStruct) == -1) { // -1 : fail
...
template <typename T>
std::string GetSandboxFileName(const std::string& outputFile, T sessionConfig)
{
    std::string fileName;
    if (!outputFile.empty()) {
        fileName = outputFile;
        ...
        fileName = SANDBOX_PATH_ROOT + "hiprofiler_data.htrace";
...
template <typename T>
bool HandleSandboxFilename(const std::string& outputFile, T sessionConfig)
{
                                                        ...
                                                        const std::string& outputFile)
{
    ...
    if (!HandleSandboxFilename(outputFile, sessionConfig)) {
        return nullptr;
    }
#else
    if (!outputFile.empty()) {
...
bool SendExitFlagProcess(bool isNonBlock)
{
    if (isNonBlock) {
        close(g_pipefd[0]);
        uint32_t sigFlag = 1;
        int retVal = TEMP_FAILURE_RETRY(write(g_pipefd[1], &sigFlag, sizeof(sigFlag)));
        if (retVal <= 0) {
...
bool DoCapture(const std::string& config, const std::string& keepSecond, const std::string& outputFile, bool isNonBlock)
{
    auto profilerStub = GetProfilerServiceStub();
    if (profilerStub == nullptr) {
        printf("Get profiler service stub failed!\n");
    ...
    if (!SendExitFlagProcess(isNonBlock)) {
        StopThread(keepSessionThread, sendHeart);
        return false;
    }
    if (!g_exitProcessFlag) {
...
void ParseCmdline(CommandLine& cmdLine, DataContext& data)
{
    ...
    cmdLine.AddParamSwitch("--nonblock", "", data.isNonBlock, "start capture without block");
}
...
void StartCaptureTraceFile(std::string& config,  DataContext& data)
{
    if (DoCapture(config, data.traceKeepSecond, data.outputFile, data.isNonBlock)) {
        printf("DONE\n");
    }
...
bool ParseConfig(const std::string& configFile, std::string& config)
{
    ...
    if (configFile.find('/') == std::string::npos) {
        if (adaptSandbox) {
            ...
            configFileWithPath = defaultPath + configFile; // add default path
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    if (data.isNonBlock) {
        ...
        if (childPid == 0) {
            ...
            StartCaptureTraceFile(config, data);
        } else if (childPid > 0) {
            close(g_pipefd[1]);
            uint32_t exitFlag = 0;
            ssize_t retVal = TEMP_FAILURE_RETRY(read(g_pipefd[0], &exitFlag, sizeof(exitFlag)));
            ...
            if (exitFlag == 1) {  // 1: exitance flag
                printf("Running in nonblock mode: tracing %s s....\n", data.traceKeepSecond.c_str());
