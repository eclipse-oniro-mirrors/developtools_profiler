...
#include "xpower_plugin_config_standard.pb.h"
#include "ffrt_profiler_config_standard.pb.h"
#include "network_profiler_config_standard.pb.h"
...
{
    bool ret = false;
    if (pluginConfigMap.find(pluginName) == pluginConfigMap.end()) {
    ...
    } else if (pluginName == "ffrt-profiler") {
        ret = SetSerializeFfrtProfilerConfig(pluginName, pluginConfig);
    } else if (pluginName == "network-profiler") {
        ret = SetSerializeNetworkProfilerConfig(pluginName, pluginConfig);
    } else {
...
bool ParsePluginConfig::SetSerializeFtraceConfig(const std::string& pluginName, ProfilerPluginConfig& pluginConfig)
{
    ...
    pluginConfig.set_config_data(static_cast<const void*>(configNoLiteDataVec.data()), configNoLiteDataVec.size());
    return true;
}
...
bool ParsePluginConfig::CheckRestraceTag(std::unique_ptr<ForStandard::NativeHookConfig>& hookConfig)
{
    static std::vector<std::string> resTagVect = {
        TAG_RES_FD_OPEN,         TAG_RES_FD_EPOLL,      TAG_RES_FD_EVENTFD,     TAG_RES_FD_SOCKET,
        TAG_RES_FD_PIPE,         TAG_RES_FD_DUP,        TAG_RES_FD_ALL,         TAG_RES_THREAD_PTHREAD,
        TAG_RES_THREAD_ALL,      TAG_RES_GPU_VK,        TAG_RES_GPU_GLES_IMAGE, TAG_RES_GPU_GLES_BUFFER,
        TAG_RES_GPU_CL_IMAGE,    TAG_RES_GPU_CL_BUFFER, TAG_RES_GPU_ALL,        TAG_RES_DMABUF,
        TAG_RES_ARKTS_HEAP_MASK, TAG_RES_JS_HEAP_MASK,  TAG_RES_KMP_HEAP_MASK,  TAG_RES_SO_MASK,
        TAG_RES_ASHMEM_MASK,     TAG_RES_RN_HEAP_MASK,  TAG_RES_DMABUF_MASK,    TAG_RES_ARK_GLOBAL_HANDLE};
    for (int i = 0; i < hookConfig->restrace_tag_size(); ++i) {
        if (std::find(resTagVect.begin(), resTagVect.end(), hookConfig->restrace_tag(i)) == resTagVect.end()) {
            printf("invalid restrace tag: %s\n", hookConfig->restrace_tag(i).c_str());
            return false;
        }
    }
    return true;
...
bool ParsePluginConfig::SetSerializeHookConfig(const std::string& pluginName, ProfilerPluginConfig& pluginConfig)
{
    ...
    if (!CheckRestraceTag(hookConfigNolite)) {
        return false;
    }
...
uint32_t ParsePluginConfig::GetHiperfPluginDuration()
{
    if (pluginConfigMap.find("hiperf-plugin") == pluginConfigMap.end()) {
        return 0;
    }
    std::string configData = pluginConfigMap["hiperf-plugin"];
    auto hiperfConfigNolite = std::make_unique<ForStandard::HiperfPluginConfig>();
    if (!parser_.ParseFromString(configData, hiperfConfigNolite.get())) {
        printf("hiperf config parse failed!\n");
        return 0;
    }
    return hiperfConfigNolite->duration();
}
...
{
    std::string configData = pluginConfigMap[pluginName];
    auto xpowerConfigNolite = std::make_unique<ForStandard::XpowerConfig>();
    if (!parser_.ParseFromString(configData, xpowerConfigNolite.get())) {
        return false;
    }

    std::vector<uint8_t> configDataVec(xpowerConfigNolite->ByteSizeLong());
    if (xpowerConfigNolite->SerializeToArray(configDataVec.data(), configDataVec.size()) <= 0) {
        return false;
    }
    pluginConfig.set_config_data(static_cast<const void*>(configDataVec.data()), configDataVec.size());
    return true;
...
bool ParsePluginConfig::SetSerializeFfrtProfilerConfig(const std::string& pluginName,
    ProfilerPluginConfig& pluginConfig)
{
    ...
    std::string configData = pluginConfigMap[pluginName];
    auto ffrtConfigNolite = std::make_unique<ForStandard::FfrtProfilerConfig>();
    if (!parser_.ParseFromString(configData, ffrtConfigNolite.get())) {
        return false;
    }

    std::vector<uint8_t> configDataVec(ffrtConfigNolite->ByteSizeLong());
    if (ffrtConfigNolite->SerializeToArray(configDataVec.data(), configDataVec.size()) <= 0) {
        return false;
    }
    pluginConfig.set_config_data(static_cast<const void*>(configDataVec.data()), configDataVec.size());
    return true;
...
bool ParsePluginConfig::SetSerializeNetworkProfilerConfig(const std::string& pluginName,
    ProfilerPluginConfig& pluginConfig)
{
    if (pluginConfigMap.find(pluginName) == pluginConfigMap.end()) {
        return false;
    ...
    std::string configData = pluginConfigMap[pluginName];
    auto networkConfigNolite = std::make_unique<ForStandard::NetworkProfilerConfig>();
    if (!parser_.ParseFromString(configData, networkConfigNolite.get())) {
        return false;
    }
