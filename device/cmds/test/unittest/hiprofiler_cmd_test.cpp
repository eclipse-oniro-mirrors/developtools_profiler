...
public:
    ...
    bool CheckPluginsGroup(const std::string& group)
    {
        std::string pidStr = "";
        std::string getPidCmd = "pidof hiprofiler_plugins";
        int maxRetry = 10;
        while (pidStr.empty() && maxRetry > 0) {
            RunCommand(getPidCmd, pidStr);
            maxRetry--;
        }
        ...
        int processPid = 0;
        ss >> processPid;
        std::string filePath = "/proc/" + std::to_string(processPid) + "/status";
        std::ifstream statusFile;
        std::string line;
        std::string groupsLine;
        statusFile.open(filePath);
        CHECK_TRUE(statusFile.is_open(), false, "open hiprofiler_plugins status File failed.");
        ...
        statusFile.close();
        CHECK_TRUE(!groupsLine.empty(), false, "read hiprofiler_plugins groups line failed.");
        CHECK_TRUE(groupsLine.find(group) != std::string::npos, false, "check hiprofiler_plugins log group failed.");
        return true;
    }
    ...
    std::string CreateCommand(const std::string &outFile, int time, bool isNonblock = false) const
    {
        std::string cmdStr =
            "hiprofiler_cmd \\\n"
            "-c - \\\n";
    ...
    std::string CreateHiperfCommand(const std::string &outFile, int time, bool isNonblock = false) const
    {
        std::string cmdStr =
            "hiprofiler_cmd \\\n"
            "-c - \\\n";
    ...
    std::string CreateEncoderCommand(const std::string &outFile, int time, bool isNonblock = false) const
    {
        std::string cmdStr =
            "hiprofiler_cmd \\\n"
            "-c - \\\n";
    ...
    std::string CreateSplitHtraceCommand(const std::string &outFile, int time, bool isNonblock = false) const
    {
        std::string cmdStr =
            "hiprofiler_cmd -s -k \\\n"
            "-c - \\\n";
    ...
    std::string CreateSplitNetworkProfilerCommand(const std::string &outFile, int time, bool isNonblock = false) const
    {
        std::string cmdStr =
            "hiprofiler_cmd -s -k \\\n"
            "-c - \\\n";
    ...
    std::string CreateSplitHiperfCommand(const std::string &outFile, const std::string &perfFile,
                                    const std::string &perfSplitFile, int time, bool isNonblock = false) const
    {
        std::string cmdStr =
            "hiprofiler_cmd -s -k \\\n"
            "-c - \\\n";
 ...
 * @tc.type: FUNC
 */
HWTEST_F(HiprofilerCmdTest, DFX_DFR_Hiprofiler_0110, Function | MediumTest | Level0)
{
 ...
 * @tc.type: FUNC
 */
HWTEST_F(HiprofilerCmdTest, DFX_DFR_Hiprofiler_0120, Function | MediumTest | Level0)
{
    StopProcessStub(DEFAULT_HIPROFILERD_PATH);
    sleep(1);
    StopProcessStub(DEFAULT_HIPROFILER_PLUGINS_PATH);
 ...
 * @tc.type: FUNC
 */
HWTEST_F(HiprofilerCmdTest, DFX_DFR_Hiprofiler_0130, Function | MediumTest | Level0)
{
 ...
 * @tc.type: FUNC
 */
HWTEST_F(HiprofilerCmdTest, DFX_DFR_Hiprofiler_0140, Function | MediumTest | Level0)
{
    StopProcessStub(DEFAULT_HIPROFILERD_PATH);
    sleep(1);
    StopProcessStub(DEFAULT_HIPROFILER_PLUGINS_PATH);
 ...
 * @tc.type: FUNC
 */
HWTEST_F(HiprofilerCmdTest, DFX_DFR_Hiprofiler_0150, Function | MediumTest | Level0)
{
    StopProcessStub(DEFAULT_HIPROFILERD_PATH);
    sleep(1);
    StopProcessStub(DEFAULT_HIPROFILER_PLUGINS_PATH);
 ...
 * @tc.type: FUNC
 */
HWTEST_F(HiprofilerCmdTest, DFX_DFR_Hiprofiler_0160, Function | MediumTest | Level0)
{
    StopProcessStub(DEFAULT_HIPROFILERD_PATH);
    sleep(1);
    StopProcessStub(DEFAULT_HIPROFILER_PLUGINS_PATH);
 ...
 * @tc.type: FUNC
 */
HWTEST_F(HiprofilerCmdTest, DFX_DFR_Hiprofiler_0170, Function | MediumTest | Level0)
{
    std::string cmd = "cp " + DEFAULT_SO_PATH + "libftrace_plugin.z.so " + DEFAULT_PATH;
    system(cmd.c_str());
 ...
 * @tc.type: FUNC
 */
HWTEST_F(HiprofilerCmdTest, DFX_DFR_Hiprofiler_0180, Function | MediumTest | Level0)
{
    std::string content = "";
 ...
 * @tc.type: FUNC
 */
HWTEST_F(HiprofilerCmdTest, DFX_DFR_Hiprofiler_0190, Function | MediumTest | Level0)
{
 ...
 * @tc.type: FUNC
 */
HWTEST_F(HiprofilerCmdTest, DFX_DFR_Hiprofiler_0200, Function | MediumTest | Level0)
{
 ...
 * @tc.type: FUNC
 */
HWTEST_F(HiprofilerCmdTest, DFX_DFR_Hiprofiler_0220, Function | MediumTest | Level0)
{
 ...
 * @tc.type: FUNC
 */
HWTEST_F(HiprofilerCmdTest, DFX_DFR_Hiprofiler_0240, Function | MediumTest | Level0)
{
    std::string outFileName = "split_htrace";
    std::string outFile = DEFAULT_PATH + outFileName + ".htrace";
    std::string content = "";
...
/**
 * @tc.name: hiprofiler_cmd
 * @tc.desc: Test hiprofiler_cmd with test -s -c.
 * @tc.type: FUNC
 */
HWTEST_F(HiprofilerCmdTest, DFX_DFR_Hiprofiler_0250, Function | MediumTest | Level0)
{
    StopProcessStub(DEFAULT_HIPROFILERD_PATH);
    sleep(1);
    StopProcessStub(DEFAULT_HIPROFILER_PLUGINS_PATH);
    ...
    std::string cmd = DEFAULT_HIPROFILER_CMD_PATH + " test -c ";
    std::string content = "";
    EXPECT_TRUE(RunCommand(cmd, content));
    std::string destStr = "invalid param";
    EXPECT_TRUE(content.find(destStr) != std::string::npos);
    cmd = DEFAULT_HIPROFILER_CMD_PATH + " XXX%$ --config ";
    content = "";
    EXPECT_TRUE(RunCommand(cmd, content));
    EXPECT_TRUE(content.find(destStr) != std::string::npos);
    cmd = DEFAULT_HIPROFILER_CMD_PATH + " @X8&C -s ";
    content = "";
    EXPECT_TRUE(RunCommand(cmd, content));
    destStr = "unknown param";
    EXPECT_TRUE(content.find(destStr) != std::string::npos);
}

/**
 * @tc.name: hiprofiler_cmd
 * @tc.desc: Test hiprofiler_cmd with -b ftrace plugin.
 * @tc.type: FUNC
 */
HWTEST_F(HiprofilerCmdTest, DFX_DFR_Hiprofiler_0260, Function | MediumTest | Level0)
{
    std::string content = "";
    StartServerStub(DEFAULT_HIPROFILERD_PATH);
    sleep(1);
    ...
    sleep(1);
    std::string outFile = DEFAULT_PATH + "trace.htrace";
    int time = 3;  // 3: sleep 3s
    std::string cmd = CreateCommand(outFile, time, true);
    EXPECT_TRUE(RunCommand(cmd, content));
    sleep(time);
    EXPECT_EQ(access(outFile.c_str(), F_OK), 0);
    cmd = "rm " + outFile;
...
/**
 * @tc.name: hiprofiler_cmd
 * @tc.desc: Test hiprofiler_cmd with -b hiperf plugin.
 * @tc.type: FUNC
 */
HWTEST_F(HiprofilerCmdTest, DFX_DFR_Hiprofiler_0270, Function | MediumTest | Level0)
{
    std::string content = "";
    StartServerStub(DEFAULT_HIPROFILERD_PATH);
    sleep(1);
    ...
    sleep(1);
    std::string outFile = DEFAULT_PATH + "trace.htrace";
    int time = 10;  // 10: sleep 10s
    std::string cmd = CreateHiperfCommand(outFile, time, true);
    EXPECT_TRUE(RunCommand(cmd, content));
    sleep(time);
    EXPECT_EQ(access(outFile.c_str(), F_OK), 0);
    cmd = "rm " + outFile;
...
/**
 * @tc.name: hiprofiler_cmd
 * @tc.desc: Test hiprofiler_cmd with log group.
 * @tc.type: FUNC
 */
HWTEST_F(HiprofilerCmdTest, DFX_DFR_Hiprofiler_0280, Function | MediumTest | Level0)
{
    std::string content = "";
    StartServerStub(DEFAULT_HIPROFILERD_PATH);
    sleep(1);
    ...
    std::string outFile = DEFAULT_PATH + "trace.htrace";
    int time = 10;
    std::string cmd = CreateHiperfCommand(outFile, time);
    std::unique_ptr<FILE, decltype(&pclose)> pipe(popen(cmd.c_str(), "r"), pclose);
    EXPECT_NE(pipe, nullptr);
    std::string logGroupId = "1007";
    EXPECT_TRUE(CheckPluginsGroup(logGroupId));
    std::array<char, READ_BUFFER_SIZE> buffer;
    while (fgets(buffer.data(), buffer.size(), pipe.get()) != nullptr) {
        content += buffer.data();
    }
...
/**
 * @tc.name: hiprofiler_cmd
 * @tc.desc: Test hiprofiler_cmd with long option.
 * @tc.type: FUNC
 */
HWTEST_F(HiprofilerCmdTest, DFX_DFR_Hiprofiler_0290, Function | MediumTest | Level0)
{
    StopProcessStub(DEFAULT_HIPROFILERD_PATH);
    sleep(1);
    StopProcessStub(DEFAULT_HIPROFILER_PLUGINS_PATH);
    ...
    std::string cmd = DEFAULT_HIPROFILER_CMD_PATH + " --start ";
    std::string content = "";
    EXPECT_TRUE(RunCommand(cmd, content));
    std::string destStr = "invalid param";
    EXPECT_FALSE(content.find(destStr) != std::string::npos);
    cmd = DEFAULT_HIPROFILER_CMD_PATH + " --start --kill ";
    content = "";
    EXPECT_TRUE(RunCommand(cmd, content));
    EXPECT_FALSE(content.find(destStr) != std::string::npos);
    cmd = DEFAULT_HIPROFILER_CMD_PATH + " --start --time 30 --kill";
    content = "";
    EXPECT_TRUE(RunCommand(cmd, content));
    EXPECT_FALSE(content.find(destStr) != std::string::npos);
    cmd = DEFAULT_HIPROFILER_CMD_PATH + " --start --time 30 --kill --out /data/local/tmp/trace.htrace";
    content = "";
    EXPECT_TRUE(RunCommand(cmd, content));
    EXPECT_FALSE(content.find(destStr) != std::string::npos);
    cmd = DEFAULT_HIPROFILER_CMD_PATH + " --start --kill --list";
    content = "";
    EXPECT_TRUE(RunCommand(cmd, content));
    EXPECT_FALSE(content.find(destStr) != std::string::npos);
}