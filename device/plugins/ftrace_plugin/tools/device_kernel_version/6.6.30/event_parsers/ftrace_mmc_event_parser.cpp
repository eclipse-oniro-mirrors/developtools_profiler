...
template <typename T>
void mmc_request_done_func(T &msg, uint8_t data[], size_t size, const EventFormat &format)
{
    int i = 0;
...
REGISTER_FTRACE_EVENT_PARSE_FUNCTION(mmc_request_done, [](FtraceEvent &ftraceEvent, uint8_t data[], size_t size,
                                                          const EventFormat &format) {
    auto msg = ftraceEvent.mutable_mmc_request_done_format();
    mmc_request_done_func(msg, data, size, format);
});
REGISTER_FTRACE_EVENT_PARSE_ENCODER_FUNCTION(mmc_request_done,
...
template <typename T>
void mmc_request_start_func(T &msg, uint8_t data[], size_t size, const EventFormat &format)
{
    int i = 0;
    msg->set_cmd_opcode(FtraceFieldParser::ParseIntField<uint32_t>(format.fields, i++, data, size));
...
REGISTER_FTRACE_EVENT_PARSE_FUNCTION(mmc_request_start, [](FtraceEvent &ftraceEvent, uint8_t data[], size_t size,
                                                           const EventFormat &format) {
    auto msg = ftraceEvent.mutable_mmc_request_start_format();
    mmc_request_start_func(msg, data, size, format);
});
REGISTER_FTRACE_EVENT_PARSE_ENCODER_FUNCTION(mmc_request_start,
                                             [](ProtoEncoder::FtraceEvent &ftraceEvent, uint8_t data[], size_t size,
                                                const EventFormat &format) {
                                                auto msg = ftraceEvent.mutable_mmc_request_start_format();
                                                mmc_request_start_func(msg, data, size, format);
