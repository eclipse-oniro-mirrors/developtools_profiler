...
template <typename T>
void signal_deliver_func(T &msg, uint8_t data[], size_t size, const EventFormat &format)
{
    int i = 0;
...
REGISTER_FTRACE_EVENT_PARSE_FUNCTION(signal_deliver, [](FtraceEvent &ftraceEvent, uint8_t data[], size_t size,
                                                        const EventFormat &format) {
    auto msg = ftraceEvent.mutable_signal_deliver_format();
    signal_deliver_func(msg, data, size, format);
});
REGISTER_FTRACE_EVENT_PARSE_ENCODER_FUNCTION(signal_deliver, [](ProtoEncoder::FtraceEvent &ftraceEvent, uint8_t data[],
...
template <typename T>
void signal_generate_func(T &msg, uint8_t data[], size_t size, const EventFormat &format)
{
    int i = 0;
    msg->set_sig(FtraceFieldParser::ParseIntField<int32_t>(format.fields, i++, data, size));
    msg->set_error_code(FtraceFieldParser::ParseIntField<int32_t>(format.fields, i++, data, size));
...
REGISTER_FTRACE_EVENT_PARSE_FUNCTION(signal_generate, [](FtraceEvent &ftraceEvent, uint8_t data[], size_t size,
                                                         const EventFormat &format) {
    auto msg = ftraceEvent.mutable_signal_generate_format();
    signal_generate_func(msg, data, size, format);
});
REGISTER_FTRACE_EVENT_PARSE_ENCODER_FUNCTION(signal_generate, [](ProtoEncoder::FtraceEvent &ftraceEvent, uint8_t data[],
                                                                 size_t size, const EventFormat &format) {
    auto msg = ftraceEvent.mutable_signal_generate_format();
    signal_generate_func(msg, data, size, format);
});
