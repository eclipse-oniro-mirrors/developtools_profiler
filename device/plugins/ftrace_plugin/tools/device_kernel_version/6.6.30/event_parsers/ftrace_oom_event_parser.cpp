...
template <typename T>
void oom_score_adj_update_func(T &msg, uint8_t data[], size_t size, const EventFormat &format)
{
    int i = 0;
...
REGISTER_FTRACE_EVENT_PARSE_FUNCTION(oom_score_adj_update, [](FtraceEvent &ftraceEvent, uint8_t data[], size_t size,
                                                              const EventFormat &format) {
    auto msg = ftraceEvent.mutable_oom_score_adj_update_format();
    oom_score_adj_update_func(msg, data, size, format);
});
REGISTER_FTRACE_EVENT_PARSE_ENCODER_FUNCTION(oom_score_adj_update,
