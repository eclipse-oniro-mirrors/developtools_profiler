...
template <typename T>
void task_newtask_func(T &msg, uint8_t data[], size_t size, const EventFormat &format)
{
    int i = 0;
...
REGISTER_FTRACE_EVENT_PARSE_FUNCTION(task_newtask, [](FtraceEvent &ftraceEvent, uint8_t data[], size_t size,
                                                      const EventFormat &format) {
    auto msg = ftraceEvent.mutable_task_newtask_format();
    task_newtask_func(msg, data, size, format);
});
REGISTER_FTRACE_EVENT_PARSE_ENCODER_FUNCTION(task_newtask, [](ProtoEncoder::FtraceEvent &ftraceEvent, uint8_t data[],
...
template <typename T>
void task_rename_func(T &msg, uint8_t data[], size_t size, const EventFormat &format)
{
    int i = 0;
    msg->set_pid(FtraceFieldParser::ParseIntField<int32_t>(format.fields, i++, data, size));
...
REGISTER_FTRACE_EVENT_PARSE_FUNCTION(task_rename, [](FtraceEvent &ftraceEvent, uint8_t data[], size_t size,
                                                     const EventFormat &format) {
    auto msg = ftraceEvent.mutable_task_rename_format();
    task_rename_func(msg, data, size, format);
});
REGISTER_FTRACE_EVENT_PARSE_ENCODER_FUNCTION(task_rename, [](ProtoEncoder::FtraceEvent &ftraceEvent, uint8_t data[],
                                                             size_t size, const EventFormat &format) {
    auto msg = ftraceEvent.mutable_task_rename_format();
    task_rename_func(msg, data, size, format);
});
