...
template <typename T>
void gpu_mem_total_func(T& msg, uint8_t data[], size_t size, const EventFormat& format)
{
    int i = 0;
...
REGISTER_FTRACE_EVENT_PARSE_FUNCTION(gpu_mem_total, [](FtraceEvent& ftraceEvent, uint8_t data[], size_t size,
                                                       const EventFormat& format) {
    auto msg = ftraceEvent.mutable_gpu_mem_total_format();
    gpu_mem_total_func(msg, data, size, format);
});
REGISTER_FTRACE_EVENT_PARSE_ENCODER_FUNCTION(gpu_mem_total, [](ProtoEncoder::FtraceEvent& ftraceEvent, uint8_t data[],
