        ...
        int len =
            snprintf_s(buffer, BUFFER_SIZE, BUFFER_SIZE - 1,
                    ...
                    " period=%" PRIu64 " think=%" PRIu64 " cgroup_ino=%" PRId32 "",
                       ...
                       msg.dirty_ratelimit(), msg.task_ratelimit(), msg.dirtied(), msg.dirtied_pause(), msg.paused(),
                       msg.pause(), msg.period(), msg.think(), msg.cgroup_ino());
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(balance_dirty_pages) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(balance_dirty_pages) msg snprintf_s format error ");
            return "";
        }
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(bdi_dirty_ratelimit) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(bdi_dirty_ratelimit) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(global_dirty_state) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(global_dirty_state) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    wbc_writepage, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_wbc_writepage_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        auto msg = event.wbc_writepage_format();
        char buffer[BUFFER_SIZE] = {0};
        int len = snprintf_s(buffer, BUFFER_SIZE, BUFFER_SIZE - 1,
                             "wbc_writepage: bdi %s: towrt=%" PRIu64 " skip=%" PRIu64
                        " mode=%d kupd=%d bgrd=%d reclm=%d cyclic=%d start=0x%" PRIx64 " end=0x%" PRIx64
                        " cgroup_ino=%" PRId32 "",
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(wbc_writepage) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(wbc_writepage) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(writeback_bdi_register) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        char buffer[BUFFER_SIZE] = {0};
        int len = snprintf_s(buffer, BUFFER_SIZE, BUFFER_SIZE - 1,
                             "writeback_dirty_inode: bdi %s: ino=%" PRIu64 " state=%s flags=%s", msg.name().c_str(),
                             msg.ino(),
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(writeback_dirty_inode) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "the contents of event(writeback_dirty_inode_enqueue) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        char buffer[BUFFER_SIZE] = {0};
        int len = snprintf_s(buffer, BUFFER_SIZE, BUFFER_SIZE - 1,
                             "writeback_dirty_inode_start: bdi %s: ino=%" PRIu64 " state=%s flags=%s",
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "the contents of event(writeback_dirty_inode_start) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    writeback_exec, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_writeback_exec_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        int len = snprintf_s(
            ...
            __print_symbolic(msg.reason(), {0, "background"}, {1, "vmscan"}, {2, "sync"}, {3, "periodic"},
                             {4, "laptop_timer"}, {5, "fs_free_space"}, {6, "forker_thread"}, {7, "foreign_flush"}),
            msg.cgroup_ino());
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(writeback_exec) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(writeback_exec) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(writeback_lazytime) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(writeback_lazytime) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(writeback_lazytime_iput) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        char buffer[BUFFER_SIZE] = {0};
        int len = snprintf_s(buffer, BUFFER_SIZE, BUFFER_SIZE - 1,
                             "writeback_mark_inode_dirty: bdi %s: ino=%" PRIu64 " state=%s flags=%s",
                             msg.name().c_str(), msg.ino(),
                             __print_flags(msg.state(), "|", {(1 << 0), "I_DIRTY_SYNC"}, {(1 << 1), "I_DIRTY_DATASYNC"},
                                           {(1 << 2), "I_DIRTY_PAGES"}, {(1 << 3), "I_NEW"}, {(1 << 4), "I_WILL_FREE"},
                                           {(1 << 5), "I_FREEING"}, {(1 << 6), "I_CLEAR"}, {(1 << 7), "I_SYNC"},
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "the contents of event(writeback_mark_inode_dirty) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(writeback_pages_written) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    writeback_queue, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_writeback_queue_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(writeback_queue) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(writeback_queue) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(writeback_queue_io) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(writeback_queue_io) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "the contents of event(writeback_sb_inodes_requeue) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
            ...
            snprintf_s(buffer, BUFFER_SIZE, BUFFER_SIZE - 1,
                    ...
                    " index=%" PRIu64 " to_write=%" PRIu64 " wrote=%" PRIu64 " cgroup_ino=%" PRId32 "",
                       ...
                       __print_flags(msg.state(), "|", {(1 << 0), "I_DIRTY_SYNC"}, {(1 << 1), "I_DIRTY_DATASYNC"},
                                     ...
                                     {(1 << 5), "I_FREEING"}, {(1 << 6), "I_CLEAR"}, {(1 << 7), "I_SYNC"},
                                     {(1 << 11), "I_DIRTY_TIME"}, {(1 << 8), "I_REFERENCED"}),
                       msg.dirtied_when(), (jiffies - msg.dirtied_when()) / 300, msg.writeback_index(),
                       msg.nr_to_write(), msg.wrote(), msg.cgroup_ino());
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(
                LOG_CORE, "maybe, the contents of print event(writeback_single_inode) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(writeback_single_inode) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        int len = snprintf_s(buffer, BUFFER_SIZE, BUFFER_SIZE - 1,
                        ...
                        " age=%" PRIu64 " index=%" PRIu64 " to_write=%" PRIu64 " wrote=%" PRIu64
                        " cgroup_ino=%" PRId32 "",
                             msg.name().c_str(), msg.ino(),
                             __print_flags(msg.state(), "|", {(1 << 0), "I_DIRTY_SYNC"}, {(1 << 1), "I_DIRTY_DATASYNC"},
                                           {(1 << 2), "I_DIRTY_PAGES"}, {(1 << 3), "I_NEW"}, {(1 << 4), "I_WILL_FREE"},
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "the contents of event(writeback_single_inode_start) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    writeback_start, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_writeback_start_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(writeback_start) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(writeback_start) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    writeback_wait, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_writeback_wait_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(writeback_wait) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(writeback_wait) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        char buffer[BUFFER_SIZE] = {0};
        int len = snprintf_s(buffer, BUFFER_SIZE, BUFFER_SIZE - 1,
                             "writeback_wake_background: bdi %s: cgroup_ino=%" PRId32 "", msg.name().c_str(),
                             msg.cgroup_ino());
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(
                LOG_CORE,
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "the contents of event(writeback_wake_background) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(writeback_write_inode) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "the contents of event(writeback_write_inode_start) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(writeback_written) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(writeback_written) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
