    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(napi_gro_frags_entry) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(napi_gro_frags_entry) msg snprintf_s format error ");
            return "";
        }
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(napi_gro_receive_entry) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    net_dev_queue, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_net_dev_queue_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(net_dev_queue) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(net_dev_queue) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        int len =
            snprintf_s(buffer, BUFFER_SIZE, BUFFER_SIZE - 1,
                       "net_dev_start_xmit: dev=%s queue_mapping=%u skbaddr=%p vlan_tagged=%d vlan_proto=0x%04x "
                       "vlan_tci=0x%04x protocol=0x%04x ip_summed=%d len=%u data_len=%u network_offset=%d "
                       "transport_offset_valid=%d transport_offset=%d tx_flags=%d gso_size=%d gso_segs=%d gso_type=%#x",
                       msg.name().c_str(), msg.queue_mapping(), msg.skbaddr(), msg.vlan_tagged(), msg.vlan_proto(),
                       msg.vlan_tci(), msg.protocol(), msg.ip_summed(), msg.len(), msg.data_len(), msg.network_offset(),
                       msg.transport_offset_valid(), msg.transport_offset(), msg.tx_flags(), msg.gso_size(),
                       msg.gso_segs(), msg.gso_type());
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(net_dev_start_xmit) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(net_dev_start_xmit) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    net_dev_xmit, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_net_dev_xmit_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(net_dev_xmit) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(net_dev_xmit) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(netif_receive_skb) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(netif_receive_skb) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(netif_receive_skb_entry) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    netif_rx, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_netif_rx_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        int len = snprintf_s(buffer, BUFFER_SIZE, BUFFER_SIZE - 1, "netif_rx: dev=%s skbaddr=%p len=%u",
                             msg.name().c_str(), msg.skbaddr(), msg.len());
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE, "maybe, the contents of print event(netif_rx) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(netif_rx) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    netif_rx_entry, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_netif_rx_entry_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(netif_rx_entry) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(netif_rx_entry) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
