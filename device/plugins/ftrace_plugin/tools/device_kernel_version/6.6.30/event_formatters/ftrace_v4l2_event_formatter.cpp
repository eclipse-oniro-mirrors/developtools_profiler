...
REGISTER_FTRACE_EVENT_FORMATTER(
    v4l2_dqbuf, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_v4l2_dqbuf_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE, "maybe, the contents of print event(v4l2_dqbuf) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(v4l2_dqbuf) msg snprintf_s format error ");
            return "";
        }
...
REGISTER_FTRACE_EVENT_FORMATTER(
    v4l2_qbuf, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_v4l2_qbuf_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        int len = snprintf_s(
            ...
            msg.sequence());
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE, "maybe, the contents of print event(v4l2_qbuf) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(v4l2_qbuf) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(vb2_v4l2_buf_done) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(vb2_v4l2_buf_done) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(vb2_v4l2_buf_queue) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(vb2_v4l2_buf_queue) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    vb2_v4l2_dqbuf, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_vb2_v4l2_dqbuf_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(vb2_v4l2_dqbuf) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(vb2_v4l2_dqbuf) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    vb2_v4l2_qbuf, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_vb2_v4l2_qbuf_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(vb2_v4l2_qbuf) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(vb2_v4l2_qbuf) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
