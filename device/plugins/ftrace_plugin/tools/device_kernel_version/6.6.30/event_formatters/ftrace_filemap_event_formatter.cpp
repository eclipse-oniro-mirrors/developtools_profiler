...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        int len = snprintf_s(buffer, BUFFER_SIZE, BUFFER_SIZE - 1,
                             "file_check_and_advance_wb_err: file=%p dev=%d:%d ino=0x%" PRIx64 " old=0x%x new=0x%x",
                             msg.file(), ((unsigned int)((msg.s_dev()) >> 20)),
                             ((unsigned int)((msg.s_dev()) & ((1U << 20) - 1))), msg.i_ino(), msg.old(), msg.seq_new());
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "the contents of event(file_check_and_advance_wb_err) msg snprintf_s format error ");
            return "";
        }
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        char buffer[BUFFER_SIZE] = {0};
        int len = snprintf_s(buffer, BUFFER_SIZE, BUFFER_SIZE - 1,
                             "filemap_set_wb_err: dev=%d:%d ino=0x%" PRIx64 " errseq=0x%x",
                             ((unsigned int)((msg.s_dev()) >> 20)), ((unsigned int)((msg.s_dev()) & ((1U << 20) - 1))),
                             msg.i_ino(), msg.errseq());
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(filemap_set_wb_err) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(filemap_set_wb_err) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        char buffer[BUFFER_SIZE] = {0};
        int len = snprintf_s(buffer, BUFFER_SIZE, BUFFER_SIZE - 1,
                             "mm_filemap_add_to_page_cache: dev %" PRIu64 ":%" PRIu64 " ino %p page=%s pfn=%" PRIu64
                        " ofs=%" PRIu64 "",
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "the contents of event(mm_filemap_add_to_page_cache) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        char buffer[BUFFER_SIZE] = {0};
        int len = snprintf_s(buffer, BUFFER_SIZE, BUFFER_SIZE - 1,
                             "mm_filemap_delete_from_page_cache: dev %" PRIu64 ":%" PRIu64
                        " ino %p page=%s pfn=%" PRIu64 " ofs=%" PRIu64 "",
                             (((msg.s_dev()) >> 20)), (((msg.s_dev()) & ((1U << 20) - 1))), msg.i_ino(),
                             "0000000000000000", msg.pfn(), msg.index() << 12);
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "the contents of event(mm_filemap_delete_from_page_cache) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
