...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(dma_fence_destroy) msg snprintf_s format error ");
            return "";
        }
...
REGISTER_FTRACE_EVENT_FORMATTER(
    dma_fence_emit, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_dma_fence_emit_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        auto msg = event.dma_fence_emit_format();
        char buffer[BUFFER_SIZE] = {0};
        int len = snprintf_s(buffer, BUFFER_SIZE, BUFFER_SIZE - 1,
                             "dma_fence_emit: driver=%s timeline=%s context=%u seqno=%u", msg.driver().c_str(),
                             msg.timeline().c_str(), msg.context(), msg.seqno());
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(dma_fence_emit) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(dma_fence_enable_signal) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    dma_fence_init, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_dma_fence_init_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        auto msg = event.dma_fence_init_format();
        char buffer[BUFFER_SIZE] = {0};
        int len = snprintf_s(buffer, BUFFER_SIZE, BUFFER_SIZE - 1,
                             "dma_fence_init: driver=%s timeline=%s context=%u seqno=%u", msg.driver().c_str(),
                             msg.timeline().c_str(), msg.context(), msg.seqno());
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(dma_fence_init) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(dma_fence_signaled) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(dma_fence_signaled) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(dma_fence_wait_end) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(dma_fence_wait_end) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(dma_fence_wait_start) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(dma_fence_wait_start) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
