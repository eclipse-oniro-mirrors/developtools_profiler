...
REGISTER_FTRACE_EVENT_FORMATTER(
    clk_disable, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_clk_disable_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(clk_disable) msg snprintf_s format error ");
            return "";
        }
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(clk_disable_complete) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(clk_disable_complete) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    clk_enable, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_clk_enable_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(clk_enable) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(clk_enable_complete) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(clk_enable_complete) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    clk_prepare, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_clk_prepare_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(clk_prepare) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
                              "maybe, the contents of print event(clk_prepare_complete) msg had be cut off in outfile");
        }
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(clk_prepare_complete) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    clk_set_parent, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_clk_set_parent_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        auto msg = event.clk_set_parent_format();
        char buffer[BUFFER_SIZE] = {0};
        int len = snprintf_s(buffer, BUFFER_SIZE, BUFFER_SIZE - 1, "clk_set_parent: %s %s", msg.name().c_str(),
                             msg.pname().c_str());
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(clk_set_parent) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(clk_set_parent_complete) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    clk_set_phase, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_clk_set_phase_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        auto msg = event.clk_set_phase_format();
        char buffer[BUFFER_SIZE] = {0};
        int len = snprintf_s(buffer, BUFFER_SIZE, BUFFER_SIZE - 1, "clk_set_phase: %s %d", msg.name().c_str(),
                             (int)msg.phase());
        if (len >= BUFFER_SIZE - 1) {
            PROFILER_LOG_WARN(LOG_CORE,
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(clk_set_phase) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(clk_set_phase_complete) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    clk_set_rate, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_clk_set_rate_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(clk_set_rate) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(clk_set_rate_complete) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    clk_unprepare, [](const ForStandard::FtraceEvent& event) -> bool { return event.has_clk_unprepare_format(); },
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(clk_unprepare) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        if (len < 0) {
            PROFILER_LOG_WARN(LOG_CORE, "the contents of event(clk_unprepare_complete) msg snprintf_s format error ");
            return "";
        }
        return std::string(buffer);
    });
