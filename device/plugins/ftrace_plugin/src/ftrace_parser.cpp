...
bool FtraceParser::ParseFieldFormat(const std::string& fieldLine, EventFormat& format)
{
    ...
    if (COMMON::IsNumeric(sizeStr)) {
        fieldInfo.size = static_cast<size_t>(atoi(sizeStr.c_str()));
    }
...
bool FtraceParser::ParseFieldType(const std::string& type, FieldFormat& field)
{
    ...
    if (std::regex_match(typeName, flexDataLocArrayRegex_)) {
        CHECK_TRUE(field.size == sizeof(uint32_t), false, "__data_loc %s, size: %zu", typeName.c_str(), field.size);
...
// parse /sys/kernel/debug/tracing/saved_tgids
// refers kernel function saved_tgids_show
void FtraceParser::ParseTargetedSavedTgid(const std::string& savedTgidsPath, std::unordered_set<int>& targetPidVect)
{
    int32_t pid = 0;
    int32_t tgid = 0;
    std::stringstream sin(savedTgidsPath);
    // kernel format code with: "%d %d\n"
    while (sin >> pid >> tgid) {
        auto it = std::find(targetPidVect.begin(), targetPidVect.end(), pid);
        if (it != targetPidVect.end()) {
            tgidDict_[pid] = tgid;
        }
    }
}

void FtraceParser::ParseTargetedSaveCmdline(const std::string& savedCmdlines, std::unordered_set<int>& targetPidVect)
{
    int32_t pid;
    std::string comm;
    std::stringstream sin(savedCmdlines);
    // kernel format code with: "%d %s\n"
    while (sin >> pid >> comm) {
        auto it = std::find(targetPidVect.begin(), targetPidVect.end(), pid);
        if (it != targetPidVect.end()) {
            commDict_[pid] = comm;
...
{
    if (outSize > static_cast<size_t>(std::numeric_limits<ptrdiff_t>::max())) {
        return false;
    }
