...
using namespace OHOS::Developtools::Profiler;
    ...
    const std::string BGSRV_PROPERTY = "5456538433256433664";
} // namespace
...
void FlowController::ReloadfilterPids(std::vector<int32_t>& filterPids)
{
    for (size_t i = 0; i < traceApps_.size(); i++) {
        int pidval = 0;
        if (COMMON::IsProcessExist(traceApps_[i], pidval)) {
            filterPids.push_back(pidval);
        } else {
            PROFILER_LOG_ERROR(LOG_CORE, "ReloadTraceArgs process %s doesn't exist", traceApps_[i].c_str());
        }
    }
}
...
int FlowController::StartCapture(void)
{
    ...
    CHECK_TRUE(ParseBasicData(), -1, "parse basic data failed!");
    ...
    OHOS::HiviewDFX::UCollectClient::TraceParam traceParam {
        .bufferSize = bufferSizeKb_,
        .clockType = traceClock_.empty() ? "boot" : traceClock_
    };
    std::vector<int32_t> filterPids;
    ReloadfilterPids(filterPids);
    auto openRet = traceCollector_->OpenTrace(traceCategories_, traceParam, filterPids);
...
long FlowController::HmReadEventData()
{
    ...
    while (used < bufferSize && (nbytes = reader->Read(&buffer[used], rest)) > 0) {
        used += nbytes;
        rest -= nbytes;
    }
    if (used == bufferSize) {
...
void FlowController::HmCaptureWorkOnNomalModeInner()
{
    ...
    while (keepRunning_) {
        ...
        if (rawDataBytes == 0) {
            ...
            if (tracePeriod > duration) {
                std::this_thread::sleep_for(tracePeriod - duration);
            }
            continue;
        }
    ...
    // read last data
    long rawLastDataBytes = HmReadEventData();
    if (rawLastDataBytes > 0 && !HmParseEventDataOnNomalMode(rawLastDataBytes)) {
        PROFILER_LOG_ERROR(LOG_CORE, "HmParseEventData failed!");
    }
    tansporter_->Flush();
...
template <typename E>
bool FlowController::HmParseEventForPid(uint8_t* &data, std::unordered_set<int>& pidList)
{
    ...
    while (curPtr < endPtr) {
        ...
        int pid = ftraceParser_->ParseFtraceEventForPid(reinterpret_cast<uint8_t*>(header), evtSize, parseEventCtx);
        if (pid != 0) {
            pidList.insert(pid);
        }
        curPtr += RmqEntryTotalSize(evtSize);
...
bool FlowController::HmParseEventDataOnNomalMode(long dataSize)
{
    ...
    std::unordered_set<int> eventPidList = {};
    for (auto dataPid = buffer; dataPid < endPtr;) {
        if (resultWriter_->isProtobufSerialize) {
            HmParseEventForPid<FtraceEvent>(dataPid, eventPidList);
        } else {
            HmParseEventForPid<ProtoEncoder::FtraceEvent>(dataPid, eventPidList);
        }
    }
    // update tgidDict_
    if (!eventPidList.empty()) {
        ftraceParser_->ParseTargetedSavedTgid(FtraceFsOps::GetInstance().GetSavedTgids(), eventPidList);
        ftraceParser_->ParseTargetedSaveCmdline(FtraceFsOps::GetInstance().GetSavedCmdLines(), eventPidList);
    }
...
bool FlowController::ParseEventData(int cpuid, uint8_t* page)
{
    if (resultWriter_->isProtobufSerialize) {
        auto traceResult = std::make_unique<TracePluginResult>();
        traceResult->set_version(osVersion_);
...
int FlowController::StopCapture(void)
{
    ...
    // parse comm dict info
    CHECK_TRUE(ParseCommDictInfo(), -1, "parse commDict info FAILED!");
...
bool FlowController::ParseCommDictInfo()
{
    if (resultWriter_->isProtobufSerialize) {
        auto traceResult = std::make_unique<TracePluginResult>();
        CHECK_TRUE(ParseCommDict(traceResult), false, "parse commDict stats FAILED!");
        CHECK_TRUE(tansporter_->Submit(std::move(traceResult)), false, "report commDict stats FAILED!");
    } else {
        auto ctx = resultWriter_->startReport(resultWriter_);
        CHECK_NOTNULL(ctx, false, "%s: get RandomWriteCtx FAILED!", __func__);
        auto traceResult = std::make_unique<ProtoEncoder::TracePluginResult>(ctx);
...
template <typename T> bool FlowController::ParseCommDict(T& tracePluginResult)
{
    ...
    for (auto& [tid, comm] : ftraceParser_->GetCommDict()) {
        auto commDictMsg = tracePluginResult->add_comm_dict();
        commDictMsg->set_tid(tid);
        commDictMsg->set_comm(comm);
    }
    return true;
}
...
std::string FlowController::GetCmdArgs(const TracePluginConfig& traceConfig)
{
