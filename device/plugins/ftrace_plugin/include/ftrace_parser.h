...
public:
    ...
    void ParseTargetedSavedTgid(const std::string& savedTgid, std::unordered_set<int>& targetPidVect);
    bool ParseSavedCmdlines(const std::string& savedCmdlines);

    void SetDebugOn(bool value);
    template <typename E>
    int ParseFtraceEventForPid(uint8_t data[], size_t dataSize, E& parseEventCtx)
    {
        CHECK_TRUE(
    ...
    std::unordered_map<int32_t, std::string>& GetCommDict()
    {
        return commDict_;
    }
    void ParseTargetedSaveCmdline(const std::string& savedCmdlines, std::unordered_set<int>& targetPidVect);
...
private:
    ...
    bool ParseFtraceEvent(T& ftraceEvent, uint8_t data[], size_t dataSize, P& parseEventCtx)
    {
        ...
        if (pid != 0) {
            ...
            auto it = commDict_.find(tgid);
            if (it == commDict_.end()) {
                std::string processComm = FtraceFsOps::GetInstance().GetProcessComm(tgid);
                if (processComm.size() > 0) {
                    processComm.pop_back(); // /proc/xxx/comm end with `\n`
                    commDict_.insert(std::pair<int32_t, std::string>(tgid, processComm));
                }
            }
        }
