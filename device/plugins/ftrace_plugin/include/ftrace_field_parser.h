...
public:
    template <typename T> static T ParseIntField(const FieldFormat& format, uint8_t data[], size_t size)
    {
        ...
        size_t copySize = format.size;
        if (sizeof(T) < format.size) {
            copySize = sizeof(T);
        }
        ReadData(start, end, &retval, copySize);
        return retval;
    }

    template <typename T>
    static std::vector<T> ParseVectorIntField(const std::vector<FieldFormat>& fields,
