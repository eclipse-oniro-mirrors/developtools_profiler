 ...
 * @tc.type: FUNC
 */
HWTEST_F(FtraceParserTest, Init, TestSize.Level0)
{
    FtraceParser parser;
 ...
 * @tc.type: FUNC
 */
HWTEST_F(FtraceParserTest, SetupEvent, TestSize.Level0)
{
    FtraceParser parser;
 ...
 * @tc.type: FUNC
 */
HWTEST_F(FtraceParserTest, ParseSavedTgid, TestSize.Level0)
{
    FtraceParser parser;
 ...
 * @tc.type: FUNC
 */
HWTEST_F(FtraceParserTest, ParseSavedCmdlines, TestSize.Level0)
{
    FtraceParser parser;
 ...
 * @tc.type: FUNC
 */
HWTEST_F(FtraceParserTest, ParsePerCpuStatus, TestSize.Level0)
{
 ...
 * @tc.type: FUNC
 */
HWTEST_F(FtraceParserTest, ParsePage, TestSize.Level0)
{
    std::vector<uint8_t> buffer(PAGE_SIZE, 0);
    std::string traceRaw;
    if (FtraceFsOps::GetInstance().IsHmKernel()) {
        traceRaw = FtraceFsOps::GetInstance().GetHmRawTracePath();
    } else {
        traceRaw = FtraceFsOps::GetInstance().GetRawTracePath(0);
    }
    ...
    if (!FtraceFsOps::GetInstance().IsHmKernel()) {
        ...
        parser.Init();
        EXPECT_TRUE(parser.ParsePage(cpuDetailMsg, buffer.data(), buffer.size(), event));
        EXPECT_EQ(close(fd), 0);
    }
}

/*
 * @tc.name: TestGetCommDict
 * @tc.desc: test FtraceParser::GetCommDict with normal case.
 * @tc.type: FUNC
 */
HWTEST_F(FtraceParserTest, TestGetCommDict, TestSize.Level0)
{
    FtraceParser parser;
    auto& dict = parser.GetCommDict();
    ...
    EXPECT_EQ(dict[1], "value1");
    EXPECT_EQ(dict[2], "value2");

    auto& dict2 = parser.GetCommDict();
    EXPECT_EQ(dict2.size(), 2);
    EXPECT_EQ(dict2[1], "value1");
    EXPECT_EQ(dict2[2], "value2");

    dict2[3] = "value3";
    EXPECT_EQ(dict.size(), 3);
    EXPECT_EQ(dict[3], "value3");
}
} // namespace