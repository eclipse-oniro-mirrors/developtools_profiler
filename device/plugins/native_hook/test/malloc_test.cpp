...
static const char* TEST_FILE_NAME = "./mmapTest";
...
static void ApplyForRealloc(int mallocSize)
{
    ...
    clock_t timerStop = 0;
    double duration = 0;
    char* p = static_cast<char*>(malloc(mallocSize));
    if (!p) {
        printf("malloc failure\n");
        return;
    }
...
static char* CreateMmap(void)
{
    ...
    char* pMap = static_cast<char*>(mmap(nullptr, PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, g_fd, 0));
...
static char* RandString(int maxLength)
{
    ...
    char* data = static_cast<char*>(malloc(strLength + 1));
    if (data != nullptr) {
