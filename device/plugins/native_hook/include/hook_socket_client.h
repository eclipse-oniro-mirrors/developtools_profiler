...
public:
    HookSocketClient(int pid, ClientConfig* config, std::atomic<Range>* targetedRange = nullptr,
                    std::atomic<int>* memCount = nullptr, void (*disableHookCallback)() = nullptr);
    ~HookSocketClient();
    ...
    virtual bool SendStackWithPayload(const void* data, size_t size, const void* payload,
...
private:
    ...
    uint64_t asyncFlagBackup_ = 0;
    bool asyncFlagConfigured_ = false;

    void RestoreAsyncStackType();
};
