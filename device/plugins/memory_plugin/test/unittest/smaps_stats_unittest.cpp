...
HWTEST_F(SmapsStatsTest, TestFrameworkForPath, TestSize.Level1)
{
    ...
    uint64_t swap1 = 1;
    uint64_t swapPss1 = 2;
    uint64_t swapper1 = swap1 + swapPss1;
    uint64_t swap2 = 11152;
    uint64_t swapPss2 = 10800;
    uint64_t swapper2 = swap2 + swapPss2;
    double reside = static_cast<double>(rss) / size * PERCENT;
    uint64_t dirty2 = 4;
    uint64_t sharedClean1 = 0;
    uint64_t sharedDirty1 = 0;
    uint64_t privateClean1 = 4;
    uint64_t privateDirty1 = 0;
    ...
    ASSERT_TRUE(plugin.ParseMaps(pid, processMemoryInfo, smapsInfo, true, true, false));
    EXPECT_EQ(java_heap, (uint64_t)(plugin.GetProcessJavaHeap()));
    EXPECT_EQ(native_heap, (uint64_t)(plugin.GetProcessNativeHeap()));
    EXPECT_EQ(code, (uint64_t)(plugin.GetProcessCode()));
    EXPECT_EQ(stack, (uint64_t)(plugin.GetProcessStack()));
    ...
    EXPECT_EQ(swapper1, processMemoryInfo.smapinfo(0).swapper());
    EXPECT_EQ(reside, processMemoryInfo.smapinfo(0).reside());
    EXPECT_EQ(swap1, processMemoryInfo.smapinfo(0).swap());
    EXPECT_EQ(swapPss1, processMemoryInfo.smapinfo(0).swap_pss());
    EXPECT_EQ(sharedClean1, processMemoryInfo.smapinfo(0).shared_clean());
    EXPECT_EQ(sharedDirty1, processMemoryInfo.smapinfo(0).shared_dirty());
    EXPECT_EQ(privateClean1, processMemoryInfo.smapinfo(0).private_clean());
    EXPECT_EQ(privateDirty1, processMemoryInfo.smapinfo(0).private_dirty());
    ...
    EXPECT_EQ(swapper2, processMemoryInfo.smapinfo(1).swapper());
...
HWTEST_F(SmapsStatsTest, TestCheckFramework, TestSize.Level1)
{
    ...
    ProcessMemoryInfo processMemoryInfo;
    SmapsInfo* smapsInfo = nullptr;
    ASSERT_TRUE(targetPlugin.ParseMaps(pid, processMemoryInfo, smapsInfo, true, false, false));
...
/**
 * @tc.name: smaps_rollup stats
 * @tc.desc: test ParseMaps
 * @tc.type: FUNC
 */
HWTEST_F(SmapsStatsTest, TestParseSmapsrollup, TestSize.Level1)
{
    // 3036, 4256, 288, 748, 0, 1388
    int pid = 2;
    ...
    uint64_t swap = 0;
    uint64_t swapPss = 0;
    uint64_t swapper = swap + swapPss;
    double reside = 0;
    uint64_t sharedClean = 2800;
    uint64_t sharedDirty = 380;
    uint64_t privateClean = 320;
    uint64_t privateDirty = 1024;
    ...
    ASSERT_TRUE(plugin.ParseMaps(pid, processMemoryInfo, smapsInfo, true, true, true));
    EXPECT_EQ(size, processMemoryInfo.smapinfo(0).size());
    EXPECT_EQ(rss, processMemoryInfo.smapinfo(0).rss());
    EXPECT_EQ(pss, processMemoryInfo.smapinfo(0).pss());
    EXPECT_EQ(dirty, processMemoryInfo.smapinfo(0).dirty());
