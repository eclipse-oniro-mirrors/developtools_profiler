...
const std::string HIDUMPER_BIN_PATH = "/system/bin/hidumper";
const int WAIT_HIDUMPER_TIME = 10;
} // namespace
...
std::string MemoryDataPlugin::GetCmdArgs(const MemoryConfig& protoConfig)
{
...
{
    if (COMMON::IsNumeric(line.substr(line.find_first_of("01234567890")))) {
...
template <typename T, typename S> void MemoryDataPlugin::WriteMemoryData(T& memoryDataProto, S smapsInfo)
{
    ...
    for (int i = 0; i < protoConfig_.pid().size(); i++) {
        ...
        bool isReportSmapsrollup = protoConfig_.parse_smaps_rollup();
        if (i == 0 && (isReportApp || isReportSmaps)) {
            SmapsStats smapInfo;
            smapInfo.ParseMaps(pid, *processinfo, smapsInfo, isReportApp, isReportSmaps, isReportSmapsrollup);
            if (isReportApp) {
...
std::string MemoryDataPlugin::ReadFile(const std::string& path)
{
    ...
    while (true) {
        ...
        ssize_t nBytes = fread(&content[count], 1, content.size() - count, fp);
        if (nBytes <= 0) {
            break;
        }
    ...
    CHECK_TRUE(fclose(fp) != -1, content, "close %s failed, %d", path.c_str(), errno);
    return content;
}
...
std::string MemoryDataPlugin::RunCommand(const std::string& cmd)
{
    std::string ret = "";
    //check cmd
    if (!COMMON::CheckCmdLineArgValid(cmd)) {
        PROFILER_LOG_ERROR(LOG_CORE, "%s:the command is illegal", __func__);
        return ret;
    }
    std::vector<std::string> cmdArg;
    COMMON::SplitString(cmd, " ", cmdArg);
    cmdArg.emplace(cmdArg.begin(), HIDUMPER_BIN_PATH);

    volatile pid_t childPid = -1;
    int pipeFds[2] = {-1, -1}; // 2: pipe fds
    FILE* fp = COMMON::CustomPopen(cmdArg, "r", pipeFds, childPid);
    CHECK_NOTNULL(fp, "", "MemoryDataPlugin::RunCommand CustomPopen FAILED!r");
    std::array<char, READ_BUFFER_SIZE> buffer;
    usleep(WAIT_HIDUMPER_TIME);
...
template <typename T> void MemoryDataPlugin::ProcessGpuDumpInfo(T& dataProto, std::string& content)
{
    BufferSplitter totalBuffer((const char*)content.c_str(), content.size() + 1);
    do {
        std::string line = (totalBuffer.CurLine() != nullptr) ?
            totalBuffer.CurLine() : "";
...
template <typename T> void MemoryDataPlugin::WriteGpuDumpInfo(T& dataProto)
{
    ...
    ProcessGpuDumpInfo<T>(dataProto, content);
}
