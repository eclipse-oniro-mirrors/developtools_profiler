...
bool SmapsStats::ParseMapHead(std::string& line, MapPiecesInfo& head, SmapsHeadInfo& smapsHeadInfo)
{
    ...
    for (int i = 0; i < FIFTH_FIELD; i++) {
        ...
        if (i == 0) {
            ...
            size_t dataLen = word.size() > 1 ? word.size() - 1 : 0;
            smapsHeadInfo.permission = word.substr(0, dataLen);
...
void SmapsStats::SetPssInfoField(MemUsageInfo& memusage, std::string strfield, const char* c)
{
...
void SmapsStats::SetSharedAndSwap(MemUsageInfo& memusage, std::string strfield, const char* c)
{
...
bool SmapsStats::GetMemUsageField(std::string& line, MemUsageInfo& memusage)
{
    ...
    if (ret == 1 && *field && field[dataIndex] == ':') {
        ...
        switch (field[0]) {
            case 'P':
                SetPssInfoField(memusage, strfield, c);
                break;
            case 'S':
                SetSharedAndSwap(memusage, strfield, c);
                break;
            case 'R':
...
bool SmapsStats::GetMemFromSmapsrollup(std::string& line, MemUsageInfo& memusage)
{
    char field[64];
    int len = 0;
    const char* pLine = line.c_str();
