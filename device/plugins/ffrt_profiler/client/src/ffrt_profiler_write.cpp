...
FfrtProfilerWriter::FfrtProfilerWriter(std::string name,
    uint32_t size,
    int smbFd,
    int eventFd,
    bool block)
...
FfrtProfilerWriter::~FfrtProfilerWriter()
{
...
void FfrtProfilerWriter::Report() const
{
...
void FfrtProfilerWriter::DoStats(long bytes)
{
    ++writeCount_;
    bytesCount_ += bytes;
    bytesPending_ += bytes;
...
long FfrtProfilerWriter::Write(const void* data, size_t size)
{
...
long FfrtProfilerWriter::WriteTimeout(const void* data, size_t size, const std::function<bool()>& callback)
{
    if (shareMemoryBlock_ == nullptr || data == nullptr || size == 0) {
        return 0;
    }
...
long FfrtProfilerWriter::WriteWithPayloadTimeout(const void* data, size_t size, const void* payload, size_t payloadSize,
