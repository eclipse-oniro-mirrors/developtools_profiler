...
class WriterAdapter {
public:
    explicit WriterAdapter(bool isProtobufSerialize = true);
    ~WriterAdapter();
...
private:
    ...
    static RandomWriteCtx* StartReportFunc(WriterStruct* writer);
    static void FinishReportFunc(WriterStruct* writer, int32_t size);
