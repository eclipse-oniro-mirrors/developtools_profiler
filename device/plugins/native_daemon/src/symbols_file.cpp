...
private:
    ...
    {
        ShdrInfo shinfo;
        if (elfFile_ == nullptr || !elfFile_->GetSectionInfo(shinfo, ".eh_frame_hdr")) {
            return false;
        }
    ...
    bool LoadEhFrameHDR(const unsigned char *buffer, size_t bufferSize, uint64_t shdrOffset)
    {
        ...
        if (ehFrameHdr == nullptr) {
            HLOGD("LoadEhFrameHDR error: ehFrameHdr is null!");
            return false;
        }
...
public:
    ...
    bool IsHapAbc()
    {
        ...
        if (StringEndsWith(filePath_, ".hap") && map_ != nullptr && map_->IsMapExec()) {
