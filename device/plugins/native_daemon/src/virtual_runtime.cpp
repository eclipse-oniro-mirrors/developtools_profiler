...
bool VirtualRuntime::SymbolizeFrame(CallFrame& callFrame, pid_t pid, pid_t tid)
{
    DfxSymbol symbol;

    auto map = FindArktsMapByAddr(callFrame.ip_);
    if (!map) {
        return false;
    }

    if (map->name.empty()) {
        PROFILER_LOG_INFO(LOG_CORE, "SymbolizeFrame map->name.empty() !!! ");
        return false;
    }

    uint32_t jsfilePathId = 0;
    auto cacheIter = jsFilePathCache_.find(callFrame.ip_);
    if (cacheIter != jsFilePathCache_.end()) {
        jsfilePathId = cacheIter->second;
    } else {
        std::string_view moduleName = map->name;
        jsfilePathId = FindArkTsFilePath(moduleName);
        if (jsfilePathId != 0) {
            jsFilePathCache_.emplace(callFrame.ip_, jsfilePathId);
        }
    }

    auto found = userSymbolCache_.find(std::pair(callFrame.ip_, jsfilePathId));
    if (jsfilePathId != 0 && found != userSymbolCache_.end()) {
        symbol = found->second;
    } else {
        const perf_callchain_context context = PERF_CONTEXT_MAX;
        symbol = GetSymbol(callFrame, pid, tid, context);
    }

    callFrame.isJsFrame_ = true;

    if (!symbol.IsValid()) {
        PROFILER_LOG_INFO(LOG_CORE, "SymbolizeFrame symbol.IsValid=false !!! ");
        return false;
    }
...
std::string VirtualRuntime::ResolveMergedSoIp(uint64_t ip, const std::string& filePath)
{
    auto it = adltBaseMap_.find(filePath);
    if (it == adltBaseMap_.end()) {
        return filePath;
    }
    ...
    std::shared_lock<std::shared_mutex> lk(g_smoMapsLock);
    auto gIt = g_smoMaps.find(filePath);
    if (gIt == g_smoMaps.end()) {
        return filePath;
    }
    const auto& info = gIt->second.info;
    uint64_t relPc = ip - base;
    int foundSoIndex = -1;
    for (const auto& e : info.map_entries()) {
        if (relPc >= e.start_addr() && relPc < e.end_addr()) {
            foundSoIndex = e.so_index();
            break;
        }
    }
    ...
    std::string soName;
    for (const auto& idEntry : info.id_tlb_entries()) {
        if (idEntry.so_index() == (uint32_t)foundSoIndex) {
            soName = idEntry.name();
            break;
        }
    }
    ...
    std::string result = filePath + ":" + soName;
    return result;
}
 
void VirtualRuntime::UpdateAdltBase()
{
    for (auto& map : processMaps_) {
        if (StartsWith(map->name, "/system/lib/libadlt") &&
            EndsWith(map->name, ".so")) {
            if (adltFound_[map->name]) {
                continue;
            }
 
            if (map->offset == 0 && !map->IsMapExec()) {
                adltBaseMap_[map->name] = map->begin;
                adltFound_[map->name] = true;
            }
            EnsureSmoInfoForFile(map->name);
        }
    }
}
...
bool VirtualRuntime::IsArktsMap(const std::shared_ptr<DfxMap>& map) const
{
    if (map == nullptr) {
        PROFILER_LOG_INFO(LOG_CORE, "SymbolizeFrame IsArktsMap map empty !!! ");
        return false;
    }
    const std::string& name = map->name;

    if (IsArkJsFile(name) || (name.find("stub.an") != std::string::npos)) {
        return true;
    }
...
std::shared_ptr<DfxMap> VirtualRuntime::FindArktsMapByAddr(uint64_t ip) const
{
    if (arktsMapTree_.empty()) {
        return nullptr;
    }

    auto it = arktsMapTree_.upper_bound(ip);
    if (it == arktsMapTree_.begin()) {
        return nullptr;
    }
    ...
    if (map && ip >= map->begin && ip < map->end) {
        return map;
    }
...
uint64_t VirtualRuntime::GetAdltBase(const std::string& path) const
{
    auto it = adltBaseMap_.find(path);
    if (it != adltBaseMap_.end()) {
        return it->second;
    }
...
std::shared_ptr<DfxMap> VirtualRuntime::FindMapByPath(const std::string& path)
{
    if (processMaps_.empty()) {
        PROFILER_LOG_INFO(LOG_CORE, "[FindMapByPath] processMaps_ empty -> return nullptr");
        return nullptr;
    }
 
    for (size_t i = 0; i < processMaps_.size(); ++i) {
        auto& m = processMaps_[i];
        if (!m) {
            continue;
        }
        if (m->name == path) {
...
bool VirtualRuntime::EnsureSmoInfoForFile(const std::string& path)
{
    auto map = FindMapByPath(path);
    if (!map) {
        PROFILER_LOG_INFO(LOG_CORE, "[EnsureSmoInfo] map NOT FOUND for %s", path.c_str());
        return false;
    }
 
    auto elf = map->GetElf(pid_);
    if (!elf) {
        return false;
    }
 
    const auto& adltMaps = elf->GetAdltMap();
    const auto& adltStrtab = elf->GetAdltStrtab();
 
    {
        std::unique_lock<std::shared_mutex> lk(g_smoMapsLock);
 
        auto& entry = g_smoMaps[path];
        if (entry.initialized) {
            PROFILER_LOG_INFO(LOG_CORE, "[EnsureSmoInfo] already initialized for %s", path.c_str());
            return true;
        }
 
        SMOMapsInfo& smo = entry.info;
        smo.clear_map_entries();
        smo.clear_id_tlb_entries();
        smo.set_file_path(path);
 
        for (size_t i = 0; i < adltMaps.size(); ++i) {
            const auto& mi = adltMaps[i];
 
            auto* e = smo.add_map_entries();
            e->set_start_addr(mi.pcBegin);
            e->set_end_addr(mi.pcEnd);
            e->set_so_index(mi.psodIndex);
        }
 
        std::unordered_set<uint32_t> seen;
        for (const auto& mi : adltMaps) {
            if (seen.count(mi.psodIndex)) continue;
            seen.insert(mi.psodIndex);
 
            std::string nm;
            if (mi.nameOffset < adltStrtab.length()) {
                size_t endPos = adltStrtab.find('\0', mi.nameOffset);
                if (endPos != std::string::npos) {
                    nm = adltStrtab.substr(mi.nameOffset, endPos - mi.nameOffset);
                }
            }
 
            auto* idEntry = smo.add_id_tlb_entries();
            idEntry->set_so_index(mi.psodIndex);
            idEntry->set_name(nm);
        }
...
void VirtualRuntime::HandleMapInfo(std::vector<uint64_t> info, const std::string& filePath, pid_t pid, pid_t tid)
{
    ...
    if (hookConfig_.async_stack_enable()) {
        bool isArkTS = (filePath.find(".hap") != std::string::npos) ||
            (filePath.find("[anon:ArkTS") != std::string::npos) ||
            (filePath.find("stub.an") != std::string::npos) ||
            (filePath.find(".hsp") != std::string::npos) ||
            (filePath.find(".abc") != std::string::npos) ||
            (filePath.find(".hqf") != std::string::npos);
        if (isArkTS) {
            if (mapsCache_.find(begin) == mapsCache_.end()) {
                soBegin_ = begin;
                auto mapItem = std::make_shared<DfxMap>(begin, begin + length, offset, flags, filePath);
                auto memMaps = std::make_shared<MemMaps>(++memMapFilePathId_);
                memMaps->AddMap(mapItem, true);
    ...
    if (offset == 0 && mapsCache_.find(begin) == mapsCache_.end()) {
        ...
        if (curMapsIter != mapsCache_.end() && (curMapsIter->second->name_ == filePath)) {
            ...
            if (mapItem->name.find(".hap") != std::string::npos) {
