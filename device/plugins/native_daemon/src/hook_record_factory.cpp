...
HookRecordFactory::HookRecordFactory(NativeHookConfig hookConfig) : hookConfig_(hookConfig)
{
    ...
    for (int index = 0; index < hookRecordCacheSize; ++index) {
        ...
        arkTsMallocRecordCache_.emplace_back(std::make_shared<ArktsMallocRecord>());
        arkTsFreeRecordCache_.emplace_back(std::make_shared<ArktsFreeRecord>());
...
HookRecordPtr CreateCacheItem(int16_t type)
{
    switch (type) {
        ...
        case MALLOC_ARKTS:
            return std::make_shared<ArktsMallocRecord>();
        case FREE_ARKTS:
            return std::make_shared<ArktsFreeRecord>();
        default:
...
void HookRecordFactory::SaveRecordToCache(uint16_t type, HookRecordPtr hookRecord)
{
    switch (type) {
        ...
        case JS_STACK_MSG:
            ...
            break;
        case MALLOC_ARKTS:
            if (arkTsMallocRecordCache_.size() < HOOK_RECORD_CACHE_MAX_SIZE) {
                arkTsMallocRecordCache_.push_back(hookRecord);
            }
            break;
        case FREE_ARKTS:
            if (arkTsFreeRecordCache_.size() < HOOK_RECORD_CACHE_MAX_SIZE) {
                arkTsFreeRecordCache_.push_back(hookRecord);
            }
            break;
        default:
            break;
                                                             ...
                                                             uint32_t size, bool storeData)
{
    ...
    switch (type) {
        ...
        case MALLOC_ARKTS: {
            auto mallocRecord = GetRecordFromCache(arkTsMallocRecordCache_, MALLOC_ARKTS);
            mallocRecord->rawStack_ = rawStack;
            return mallocRecord;
        }
        case FREE_ARKTS: {
            auto freeRecord = GetRecordFromCache(arkTsFreeRecordCache_, FREE_ARKTS);
            freeRecord->rawStack_ = rawStack;
            return freeRecord;
        }
        case END_MSG:
