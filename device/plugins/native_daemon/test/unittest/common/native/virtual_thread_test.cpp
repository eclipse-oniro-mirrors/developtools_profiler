...
int VirtualThreadTest::PhdrCallBack(struct dl_phdr_info *info, size_t size, void *data)
{
    ...
    if (memMaps.size() == 0u) {
        ...
        for (const std::shared_ptr<DfxMap> &item : thread->GetMaps()) {
            HLOGV("%s", item->ToString().c_str());
        }
        return 0;
    }
 ...
 * @tc.type: FUNC
 */
HWTEST_F(VirtualThreadTest, ParseMap, TestSize.Level0)
{
    ...
    std::shared_ptr<VirtualThread> thread = std::make_shared<VirtualThread>(getpid(),
        GetThreadId(), files, runtime.get(), false);
    ...
    const std::vector<std::shared_ptr<DfxMap>> &maps = thread->GetMaps();
 ...
 * @tc.type: FUNC
 */
HWTEST_F(VirtualThreadTest, CreateMapItem, TestSize.Level0)
{
    std::unordered_map<std::string, std::unique_ptr<SymbolsFile>> files;
    std::shared_ptr<VirtualRuntime> runtime = std::make_shared<VirtualRuntime>();
 ...
 * @tc.type: FUNC
 */
HWTEST_F(VirtualThreadTest, InheritMaps, TestSize.Level0)
{
    std::unordered_map<std::string, std::unique_ptr<SymbolsFile>> files;
    ...
    const std::vector<std::shared_ptr<DfxMap>> &maps = thread->GetMaps();
    const std::vector<std::shared_ptr<DfxMap>> &maps2 = thread2->GetMaps();
    GTEST_LOG_(INFO) << maps.size();
    GTEST_LOG_(INFO) << maps2.size();
 ...
 * @tc.type: FUNC
 */
HWTEST_F(VirtualThreadTest, FindMapByAddr, TestSize.Level0)
{
    std::unordered_map<std::string, std::unique_ptr<SymbolsFile>> files;
    std::shared_ptr<VirtualRuntime> runtime = std::make_shared<VirtualRuntime>();
    std::shared_ptr<VirtualThread> thread = std::make_shared<VirtualThread>(getpid(), GetThreadId(),
 ...
 * @tc.type: FUNC
 */
HWTEST_F(VirtualThreadTest, ReadRoMemory, TestSize.Level0)
{
    ...
    if (fp) {
        ...
        const std::shared_ptr<DfxMap> map = thread->FindMapByAddr(addr);
