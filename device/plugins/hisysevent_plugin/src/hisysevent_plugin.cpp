...
std::string HisyseventPlugin::GetCmdArgs(const HisyseventConfig& protoConfig)
{
    std::string args;
...
void HisyseventPlugin::StartLoopReadData(std::unique_ptr<uint8_t[]>& buffer, std::unique_ptr<HisyseventInfo>& dataProto,
                                         std::unique_ptr<ProtoEncoder::HisyseventInfo>& hisyseventInfo)
{
    while (running_) {
        char* cptr = nullptr;
...
void HisyseventPlugin::Run(void)
{
    ...
    StartLoopReadData(buffer, dataProto, hisyseventInfo);
    if (resultWriter_->isProtobufSerialize) {
        WriteResult(dataProto.get());
        dataProto.reset();
    } else {
...
inline bool HisyseventPlugin::InitHisyseventCmd()
{
    ...
    fullCmd_.emplace_back("/system/bin/hisysevent"); // exe file path
...
inline bool HisyseventPlugin::ParseSyseventLineInfo(const char* data, size_t len, T hisyseventInfoProto)
{
    ...
    if (utf8_range::IsStructurallyValid({data, dataLen})) {
