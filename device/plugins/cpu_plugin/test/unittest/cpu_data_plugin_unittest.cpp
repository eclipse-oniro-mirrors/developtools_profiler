...
const std::string SO_PATH = "libcpudataplugin.z.so";
                       ...
                       bool skipThread = false)
{
    ...
    if (skipThread) {
        protoConfig.set_skip_thread_cpu_info(true);
    }
...
/**
 * @tc.name: cpu plugin
 * @tc.desc: cpu plugin test for skip thread.
 * @tc.type: FUNC
 */
HWTEST_F(CpuDataPluginTest, TestSkipThread, TestSize.Level1)
{
    CpuDataPlugin cpuPlugin;
    CpuData cpuData;
    g_path = g_testPath;
    ...
    g_path = g_testPath + "/proc/";
    cpuPlugin.SetPath(g_path);
    EXPECT_TRUE(PluginCpuinfoStub(cpuPlugin, cpuData, 1872, false, false, true));

    ASSERT_EQ(cpuData.thread_info_size(), 0);
    EXPECT_EQ(cpuPlugin.Stop(), 0);
...
HWTEST_F(CpuDataPluginTest, TestPluginSystemInfo, TestSize.Level1)
{
    ...
    cpuPlugin.WriteSystemCpuUsage(*cpuData.mutable_cpu_usage_info(), cpuLoadData, sysStr1.c_str(), sysStr1.length());
    CpuUsageInfo cpuUsageInfo1 = cpuData.cpu_usage_info();
    ...
    cpuPlugin.WriteSystemCpuUsage(*cpuData.mutable_cpu_usage_info(), cpuLoadData, sysStr2.c_str(), sysStr2.length());
    CpuUsageInfo cpuUsageInfo2 = cpuData.cpu_usage_info();
...
HWTEST_F(CpuDataPluginTest, TestPluginSystemBoundary, TestSize.Level1)
{
    ...
    sysStr = "";
    CpuLoadData cpuLoadData;
    cpuPlugin.WriteSystemCpuUsage(*cpuData.mutable_cpu_usage_info(), cpuLoadData, sysStr.c_str(), sysStr.length());
    CpuUsageInfo cpuUsageInfo1 = cpuData.cpu_usage_info();
    EXPECT_EQ(cpuUsageInfo1.prev_system_cpu_time_ms(), 0);
    EXPECT_EQ(cpuUsageInfo1.prev_system_boot_time_ms(), 0);
