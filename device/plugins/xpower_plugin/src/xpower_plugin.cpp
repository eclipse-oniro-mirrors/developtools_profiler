...
XpowerPlugin::XpowerPlugin()
{
    ...
    procMesTypeMapping_.insert({XpowerMessageType::THERMAL_REPORT, OptimizeMessageType::MESSAGE_THERMAL_REPORT});
}
...
std::string XpowerPlugin::GetCmdArgs(const XpowerConfig& traceConfig)
{
...
int XpowerPlugin::Start(const uint8_t *configData, uint32_t configSize)
{
    ...
    if ((messageType & OptimizeMessageType::MESSAGE_APP_DETAIL) != 0) {
        ...
        if (!isExsit) {
            ...
            COMMON::PluginWriteToHisysevent("xpower_plugin", "sh", args, COMMON::ErrorType::RET_INVALID_PID,
...
{
    if (powerClientHandle_ == nullptr) {
        powerClientHandle_ = dlopen("libxpower_manager_client.z.so", RTLD_LAZY);
    }
    if (powerClientHandle_ == nullptr) {
...
int XpowerPlugin::Stop()
{
    ...
    if (dataQueuePtr_ != nullptr) {
        dataQueuePtr_->ShutDown();
    }
    PROFILER_LOG_INFO(LOG_CORE, "%s:stop xpower plugin success", __func__);
    return 0;
}

#ifdef XPOWER_PLUGIN_UNITTEST
bool XpowerPlugin::IsListenerHandleValid()
{
    return listenerHandle_ != nullptr;
}
#endif