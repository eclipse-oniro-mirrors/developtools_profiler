...
constexpr uint32_t MAX_SIZE = INT_MAX;
...
std::pair<bool, std::string> EbpfConverter::CheckNotExistsFilePath(const std::string &filePath)
{
    ...
    if ((pathDir.length() >= PATH_MAX - 1) || (realpath(pathDir.c_str(), pathDirRealPath.get()) == nullptr)) {
        std::cout << "path is invalid:" << pathDir << std::endl;
        return {false, ""};
    }
    if (access(pathDir.c_str(), F_OK) != 0) {
        std::cout << "Fail to access path:" << pathDir << std::endl;
        return {false, ""};
    }
...
EbpfConverter::~EbpfConverter()
{
    ...
    for (iter = symbolInfo_.begin(); iter != symbolInfo_.end(); iter++) {
        ...
        if (iter->second.strTab != nullptr) {
            delete[] iter->second.strTab;
            iter->second.strTab = nullptr;
        }
    }
}
...
void EbpfConverter::EventMapsParsing()
{
    ...
    if (memset_s(fileName, sizeof(fileName), 0, sizeof(fileName)) != EOK) {
        std::cout << "memset error at EventMapsParsing" << std::endl;
        return;
    }
    if (maps.fileNameLen > 0) {
        size_t dataLen = sizeof(fileName) > 1 ? sizeof(fileName) - 1 : 0;
        CHK(Read(reinterpret_cast<void*>(fileName), dataLen));
...
void EbpfConverter::SymbolInfoParsing()
{
    ...
    if (info.strTabLen + 1 >= MAX_SIZE) {
        std::cout << "info size exceeds max size" << std::endl;
        return;
    }
    ...
    if (info.strTabLen > 0) {
        ...
        if (!Read(reinterpret_cast<void *>(info.strTab), info.strTabLen)) {
            std::cout << "read string table failed" << std::endl;
            delete[] info.strTab;
            info.strTab = nullptr;
            return;
        }
    }
    if (info.symTabLen + 1 >= MAX_SIZE) {
        std::cout << "info size exceeds max size" << std::endl;
        return;
    }
    ...
    if (info.symTabLen > 0) {
        ...
        if (!Read(reinterpret_cast<void *>(info.symTab), info.symTabLen)) {
            std::cout << "read symbol table failed" << std::endl;
            delete[] info.symTab;
            info.symTab = nullptr;
            return;
        }
    ...
    if (memset_s(fileName, sizeof(fileName), 0, sizeof(fileName)) != EOK) {
        std::cout << "memset error at SymbolInfoParsing" << std::endl;
        delete[] info.symTab;
        info.symTab = nullptr;
        return;
    }
    if (info.fileNameSize > 0) {
        size_t dataLen = sizeof(fileName) > 1 ? sizeof(fileName) - 1 : 0;
        if (!Read(reinterpret_cast<void*>(&fileName), dataLen)) {
...
void EbpfConverter::EventStrParsing()
{
    ...
    if (str.strLen > 0) {
        size_t dataLen = sizeof(fileName) > 1 ? static_cast<size_t>(sizeof(fileName) - 1) : 0;
        CHK(Read(reinterpret_cast<void*>(&fileName), dataLen));
        str.fileName = fileName;
    }
    str_.push_back(str);
    int32_t newPos = static_cast<int32_t>(str.len - SHIFT - str.strLen);
...
int32_t main(int32_t argc, char* argv[])
{
    ...
    auto retFileInput = OHOS::EBPF_TOOLS::EbpfConverter::CheckNotExistsFilePath(inputPath);
    auto retFileOutput = OHOS::EBPF_TOOLS::EbpfConverter::CheckNotExistsFilePath(outputPath);
    if (!retFileInput.first) {
        std::cout << "check input file path" << inputPath << " fail!" << std::endl;
        return 0;
    }
    if (!retFileOutput.first) {
        std::cout << "check output file path" << outputPath << " fail!" << std::endl;
        return 0;
    }
    OHOS::EBPF_TOOLS::EbpfConverter ec(retFileInput.second, retFileOutput.second);
    ec.StartParsing();
    return 0;
}