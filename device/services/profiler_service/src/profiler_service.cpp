...
#define CHECK_POINTER_NOTNULL(ptr, errorMessage)                              \
    do {                                                                      \
        if ((ptr) == nullptr) {                                                 \
            PROFILER_LOG_ERROR(LOG_CORE, "%s: FAILED, %s is null!", __func__, #ptr); \
...
#define CHECK_EXPRESSION_TRUE(expr, errorMessage)                            \
    do {                                                                     \
        if (!(expr)) {                                                       \
            PROFILER_LOG_ERROR(LOG_CORE, "%s: FAILED, %s", __func__, errorMessage); \
            return {StatusCode::INTERNAL, (errorMessage)};                   \
        }                                                                    \
    } while (0)
...
constexpr uint64_t CHECK_HEARTBEAT_INTERVAL = 500;
constexpr long BYTE_PER_KB = 1024;
...
{
    PROFILER_LOG_INFO(LOG_CORE, "~SessionContext id = %d", id);
    if (offlineScheduleTaskFd != -1) {
        stopExpireTask.UnscheduleTask(offlineScheduleTaskFd);
    }
    StopSessionExpireTask(service->removeTask_);
                                        ...
                                        ::GetCapabilitiesResponse* response)
{
    ...
    PROFILER_LOG_INFO(LOG_CORE, "GetCapabilities from '%s'", context->peer().c_str());

    PROFILER_LOG_INFO(LOG_CORE, "GetCapabilities %d start", request->request_id());
    ...
    PROFILER_LOG_INFO(LOG_CORE, "GetCapabilities %d done!", request->request_id());
    return Status::OK;
}
...
bool ProfilerService::SessionContext::CreatePluginSessions()
{
    if (bufferConfigs.size() > 0) { // with buffer configs
        CHECK_TRUE(service->pluginSessionManager_->CreatePluginSessions(pluginConfigs, bufferConfigs, dataRepeater,
            stateRepeater), false, "create plugin sessions with buffer configs failed!");
    } else { // without buffer configs
        CHECK_TRUE(service->pluginSessionManager_->CreatePluginSessions(pluginConfigs, dataRepeater, stateRepeater),
                   false, "create plugin sessions without buffer configs failed!");
    }
    return true;
}
    ...
    if (sessionConfig.session_mode() == ProfilerSessionConfig::OFFLINE) {
        ...
        if (sampleDuration > 0) {
            ...
            if (sessionConfig.keep_alive_time() == 0) {
                ...
                StartSessionExpireTask(service->removeTask_);
            }
        }
    }
...
bool ProfilerService::SessionContext::StopPluginSessions()
{
    if (sessionConfig.session_mode() == ProfilerSessionConfig::OFFLINE) {
        if (offlineScheduleTaskFd != -1) {
            stopExpireTask.UnscheduleTask(offlineScheduleTaskFd);
            offlineScheduleTaskFd = -1;
        } else {
            return true;
        }
        traceFileWriter->SetDurationTime();
    }

    // stop each plugin sessions
    service->pluginSessionManager_->StopPluginSessions(pluginNames);
    // stop epoll thread receiving shared memory messages
    service->pluginService_->StopEpollThread();

    // Read the remaining data of shared memory of all plugins.
    for (auto& name : pluginNames) {
        service->pluginService_->FlushAllData(name);
    }
    if (sessionConfig.session_mode() == ProfilerSessionConfig::OFFLINE) {
        // update file header.
...
void ProfilerService::SessionContext::StartSessionExpireTask(ScheduleTaskManager& task)
{
    if (sessionConfig.keep_alive_time() > 0 && timeoutScheduleTaskFd == -1) {
        timeoutScheduleTaskFd = task.ScheduleTask(
            std::bind(&ProfilerService::RemoveSessionContext, service, id),
            sessionConfig.keep_alive_time(), true);
    }
}

void ProfilerService::SessionContext::StopSessionExpireTask(ScheduleTaskManager& task)
{
    if (sessionConfig.keep_alive_time() > 0 && timeoutScheduleTaskFd != -1) {
        task.UnscheduleTask(timeoutScheduleTaskFd);
        timeoutScheduleTaskFd = -1;
    }
}
                                      ...
                                      ::CreateSessionResponse* response)
{
    ...
    PROFILER_LOG_INFO(LOG_CORE, "CreateSession %d start", request->request_id());
    const int nConfigs = request->plugin_configs_size();
    CHECK_EXPRESSION_TRUE(nConfigs > 0, "no plugin configs!");

    // check buffer configs
    std::shared_ptr<ProfilerSessionConfig> sessionConfig =
    std::make_shared<ProfilerSessionConfig>(request->session_config());
    ...
    if (pluginConfigs.empty()) {
        PROFILER_LOG_ERROR(LOG_CORE, "No plugins are loaded!");
    ...
    PROFILER_LOG_INFO(LOG_CORE, "bufferConfigs: %zu", bufferConfigs.size());
    ...
    //set session configs
    pluginService_->SetProfilerSessionConfig(sessionConfig, pluginNames);

    // create TraceFileWriter for offline mode
    TraceFileWriterPtr traceWriter;
    std::shared_ptr<ProfilerDataRepeater<ProfilerPluginData>> dataRepeater = nullptr;
    if (sessionConfig->session_mode() == ProfilerSessionConfig::OFFLINE) {
        ...
        for (std::vector<ProfilerPluginConfig>::size_type i = 0; i < pluginConfigs.size(); i++) {
            ...
            pluginData.set_sample_interval(request->plugin_configs(i).sample_interval());
            ...
            if (pluginData.SerializeToArray(msgData.data(), msgData.size()) <= 0) {
                PROFILER_LOG_WARN(LOG_CORE, "PluginConfig SerializeToArray failed!");
            }
            traceWriter->SetPluginConfig(msgData.data(), msgData.size());
        }
        traceWriter->Flush();
    } else {
        dataRepeater = std::make_shared<ProfilerDataRepeater<ProfilerPluginData>>(DEFAULT_REPEATER_BUFFER_SIZE);
        CHECK_POINTER_NOTNULL(dataRepeater, "alloc ProfilerDataRepeater failed!");
    }
    ProfilerPluginState* state = response->add_plugin_status();
    state->set_version(COMMON::STATE_VERSION);
    ...
    if (stateRepeater_ != nullptr) {
        ctx->stateRepeater = stateRepeater_;
    }
    ...
    if (ctx->sessionConfig.session_mode() == ProfilerSessionConfig::ONLINE) {
        if (stateRepeater_ == nullptr) {
            stateRepeater_ = std::make_shared<ProfilerDataRepeater<ProfilerPluginState>>(DEFAULT_REPEATER_BUFFER_SIZE);
        }
        if (stateRepeater_ != nullptr) {
            ctx->stateRepeater = stateRepeater_;
        }
    ...
    if (keepAliveTime) {
        ...
        ctx->StartSessionExpireTask(removeTask_);
    }
    ...
    PROFILER_LOG_INFO(LOG_CORE, "CreateSession %d %u done!", request->request_id(), sessionId);
    return Status::OK;
}
...
void ProfilerService::CheckClientStatus()
{
    int pidVal = 0;
    if (!COMMON::IsProcessExist(HIPROFILER_PLUGINS_NAME, pidVal)) {
        ProfilerPluginState pluginState;
        pluginState.set_version(COMMON::STATE_VERSION);
        pluginState.set_event_detail("hiprofiler_plugins process is dead!");
        pluginState.set_event(ProfilerPluginState::RUNNING_ERR);
        CHECK_NOTNULL(stateRepeater_, NO_RETVAL, "cannot push event, stateRepeater is null!");
        stateRepeater_->PutPluginData(std::make_shared<ProfilerPluginState>(pluginState));
        if (heartbeatFd_ != -1) {
            checkStatusManager_.UnscheduleTask(heartbeatFd_);
        }
    }
}
...
bool ProfilerService::RemoveSessionContext(uint32_t sessionId)
{
    ...
    if (it != sessionContext_.end()) {
        auto ptr = it->second;
        PROFILER_LOG_INFO(LOG_CORE, "DelCtx use_count: %ld", ptr.use_count());
    ...
    const std::string& outputFile, const std::string& pluginVersion)
{
    if (pluginName.empty() || outputFile.empty()) {
        PROFILER_LOG_ERROR(LOG_CORE, "pluginName(%s) didn't set output file(%s)",
                           pluginName.c_str(), outputFile.c_str());
        return;
    }
    auto retFile = COMMON::CheckNotExistsFilePath(outputFile);
    if (!retFile.first) {
        PROFILER_LOG_INFO(LOG_CORE, "%s:check file path %s fail", __func__, outputFile.c_str());
        return;
    }
    std::ifstream fsFile {}; // read from output file
    fsFile.open(retFile.second, std::ios_base::in | std::ios_base::binary);
    if (!fsFile.good()) {
        PROFILER_LOG_ERROR(LOG_CORE, "open file(%s) failed: %d", outputFile.c_str(), fsFile.rdstate());
        return;
    }
    auto targetFile = COMMON::CheckNotExistsFilePath(resultFile);
    if (!targetFile.first) {
        PROFILER_LOG_INFO(LOG_CORE, "%s:check file path %s fail", __func__, resultFile.c_str());
        return;
    }
    std::ofstream fsTarget {}; // write to profiler ouput file
    fsTarget.open(targetFile.second, std::ios_base::in | std::ios_base::out | std::ios_base::binary);
    if (!fsTarget.good()) {
        PROFILER_LOG_ERROR(LOG_CORE, "open file(%s) failed: %d", resultFile.c_str(), fsTarget.rdstate());
        return;
    }
    ...
    if (ret != EOK) {
        PROFILER_LOG_ERROR(LOG_CORE, "strncpy_s error! pluginName is %s", pluginName.c_str());
        return;
    }
    ...
    if (ret != EOK) {
        PROFILER_LOG_ERROR(LOG_CORE, "strncpy_s error! pluginVersion is %s", pluginVersion.c_str());
        return;
    }
    fsTarget.write(reinterpret_cast<char*>(&header), sizeof(header));
    if (!fsTarget.good()) {
        PROFILER_LOG_ERROR(LOG_CORE, "write file(%s) header failed: %d\n", resultFile.c_str(), fsTarget.rdstate());
        return;
    }
    ...
    while ((readSize = std::min(bufSize, fileSize)) > 0) {
        ...
        if (!fsTarget.good()) {
            PROFILER_LOG_ERROR(LOG_CORE, "write file(%s) failed: %d\n", resultFile.c_str(), fsTarget.rdstate());
            return;
        }
        fileSize -= readSize;
    ...
    PROFILER_LOG_INFO(LOG_CORE, "write standalone(%s) to result(%s) done", outputFile.c_str(), resultFile.c_str());
}
                                     ...
                                     ::StartSessionResponse* response)
{
    ...
    PROFILER_LOG_INFO(LOG_CORE, "StartSession %d %u start", request->request_id(), sessionId);
    ...
    for (int i = 0; i < request->update_configs_size(); i++) {
        PROFILER_LOG_INFO(LOG_CORE, "update_configs %d, name = %s", i, request->update_configs(i).name().c_str());
    ...
    // start epoll thread to receive shared memory messages.
    CHECK_EXPRESSION_TRUE(pluginService_->StartEpollThread(), "start epoll thread failed!");
    ...
    if (updateNames.size() > 0) {
        ...
        PROFILER_LOG_INFO(LOG_CORE, "StartSession %zu plugin config updated!", updates);
    }
    ...
    ProfilerPluginState* state = response->add_plugin_status();
    state->set_version(COMMON::STATE_VERSION);
    PROFILER_LOG_INFO(LOG_CORE, "StartSession %d %u done!", request->request_id(), sessionId);
    if (heartbeatFd_ == -1) {
        auto callback = [this] { this->CheckClientStatus(); };
        heartbeatFd_ = checkStatusManager_.ScheduleTask(callback, CHECK_HEARTBEAT_INTERVAL);
        if (heartbeatFd_ == -1) {
            PROFILER_LOG_INFO(LOG_CORE, "schedule task failed for heartbeat check");
        }
    }
    return Status::OK;
}
                                  ...
                                  ServerWriter<::FetchDataResponse>* writer)
{
    ...
    uint32_t sessionId = request->session_id();
    PROFILER_LOG_INFO(LOG_CORE, "FetchData %d %u start", request->request_id(), sessionId);
    ...
    if (ctx->sessionConfig.session_mode() == ProfilerSessionConfig::ONLINE) {
        ...
        while (1) {
            ...
            if (count > 0) {
                ...
                PROFILER_LOG_INFO(LOG_CORE, "no more data need to fill to response!");
            }

            bool sendSuccess = writer->Write(response);
            if (count <= 0 || !sendSuccess) {
                PROFILER_LOG_INFO(LOG_CORE, "count = %d, sendSuccess = %d", count, sendSuccess);
                break;
            }
        }
    }

    PROFILER_LOG_INFO(LOG_CORE, "FetchData %d %u done!", request->request_id(), sessionId);
    return Status::OK;
}

Status ProfilerService::SubscribeProfilerEvt(ServerContext* context,
                                             const ::SubscribeProfilerEvtRequest* request,
                                             ServerWriter<::SubscribeProfilerEvtResponse>* writer)
{
    CHECK_POINTER_NOTNULL(context, "context ptr invalid!");
    CHECK_POINTER_NOTNULL(request, "request ptr invalid!");
    CHECK_POINTER_NOTNULL(writer, "writer ptr invalid!");
    ...
    uint32_t sessionId = request->session_id();
    PROFILER_LOG_INFO(LOG_CORE, "SubscribeProfilerEvt %d %u start", request->request_id(), sessionId);
    ...
    if (ctx->sessionConfig.session_mode() == ProfilerSessionConfig::ONLINE) {
        auto stateRepeater = ctx->stateRepeater;
        CHECK_POINTER_NOTNULL(stateRepeater, "state repeater invalid!");

        while (1) {
            ctx = GetSessionContext(sessionId);
            CHECK_POINTER_NOTNULL(ctx, "session_id invalid!");

            SubscribeProfilerEvtResponse response;
            response.set_status(StatusCode::OK);
            response.set_response_id(++responseIdCounter_);

            std::vector<ProfilerPluginStatePtr> pluginStateVec;
            int count = stateRepeater->TakePluginData(pluginStateVec);
            if (count > 0) {
                response.set_has_more(true);
                for (int i = 0; i < count; i++) {
                    auto data = response.add_plugin_state();
                    CHECK_POINTER_NOTNULL(data, "new plugin data invalid");
                    CHECK_POINTER_NOTNULL(pluginStateVec[i], "plugin state invalid");
                    *data = *pluginStateVec[i];
                }
            } else {
                response.set_has_more(false);
                PROFILER_LOG_INFO(LOG_CORE, "no more data need to fill to response!");
            ...
            bool sendSuccess = writer->Write(response);
            for (auto& state : response.plugin_state()) {
                PROFILER_LOG_INFO(LOG_CORE, "sendSuccess = %d, event = %d, detail=%s",
                    sendSuccess, state.event(), state.event_detail().c_str());
            }
            if (count <= 0 || !sendSuccess) {
                PROFILER_LOG_INFO(LOG_CORE, "count = %d, sendSuccess = %d", count, sendSuccess);
                break;
    ...
    PROFILER_LOG_INFO(LOG_CORE, "SubscribeProfilerEvt %d %u done!", request->request_id(), sessionId);
    return Status::OK;
}

Status ProfilerService::StopSessionImpl(ServerContext* context,
                                        const ::StopSessionRequest* request,
                                        ::StopSessionResponse* response, uint32_t sessionId)
{
#ifdef PERFORMANCE_DEBUG
...
#endif
    CHECK_REQUEST_RESPONSE(context, request, response);
    PROFILER_LOG_INFO(LOG_CORE, "StopSession %d %u start", request->request_id(), sessionId);
    ...
    PROFILER_LOG_INFO(LOG_CORE, "StopSession %d %u done!", request->request_id(), sessionId);
#ifdef PERFORMANCE_DEBUG
    struct timespec end = {};
    clock_gettime(CLOCK_REALTIME, &end);
    uint64_t costTime = (end.tv_sec - start.tv_sec) * S_TO_NS + (end.tv_nsec - start.tv_nsec);
    PROFILER_LOG_INFO(LOG_CORE, "StopSession cost time  %" PRIu64 " ns", costTime);
#endif
    return Status::OK;
}
...
{
    if (request == nullptr) {
        return {StatusCode::INTERNAL, "request is null"};
    }
    if (request->stop_all()) {
        std::vector<uint32_t> sessionList;
        {
            std::unique_lock<std::mutex> lock(sessionContextMutex_);
            for (const auto& info : sessionContext_) {
                sessionList.push_back(info.first);
            }
        }
        for (auto sessionId : sessionList) {
            StopSessionImpl(context, request, response, sessionId);
        }
        return Status::OK;
    } else {
        uint32_t sessionId = request->session_id();
        return StopSessionImpl(context, request, response, sessionId);
    }
}

Status ProfilerService::DestroySessionImpl(ServerContext* context,
                                           const ::DestroySessionRequest* request,
                                           ::DestroySessionResponse* response, uint32_t sessionId)
{
    CHECK_REQUEST_RESPONSE(context, request, response);
    PROFILER_LOG_INFO(LOG_CORE, "DestroySession %d %u start", request->request_id(), sessionId);
    ...
    PROFILER_LOG_INFO(LOG_CORE, "DestroySession %d %u done!", request->request_id(), sessionId);

    struct stat fileInfo;
    if (stat(ctx->sessionConfig.result_file().c_str(), &fileInfo) == 0) {
        long result = static_cast<long>(fileInfo.st_size / BYTE_PER_KB);
        PROFILER_LOG_INFO(LOG_CORE, "result file %s size is %ld KB", ctx->sessionConfig.result_file().c_str(), result);
    }
                                       ...
                                       ::DestroySessionResponse* response)
{
    ...
    if (request->destroy_all()) {
        ...
        for (const auto& session : sessionContext_) {
            sessionIds.push_back(session.first);
        }
        bool success = true;
        for (const auto& sessionId : sessionIds) {
            grpc::Status status = DestroySessionImpl(context, request, response, sessionId);
            if (!status.ok()) {
                success = false;
            }
        }
        if (success) {
            ...
            return {StatusCode::INTERNAL, "ProfilerService::DestroySession failed to destroy all sessions"};
        }
    } else {
        uint32_t sessionId = request->session_id();
        return DestroySessionImpl(context, request, response, sessionId);
    }
}
                                            ...
                                            ::KeepSessionResponse* response)
{
    CHECK_REQUEST_RESPONSE(context, request, response);
    uint32_t sessionId = request->session_id();
    PROFILER_LOG_INFO(LOG_CORE, "KeepSession %d %u start", request->request_id(), sessionId);
    ...
    if (ctx->sessionConfig.keep_alive_time() > 0) {
        ctx->StopSessionExpireTask(removeTask_);
        ctx->StartSessionExpireTask(removeTask_);
    }
    PROFILER_LOG_INFO(LOG_CORE, "KeepSession %d %u done!", request->request_id(), sessionId);
    return Status::OK;
}
...
public:
    ...
    void Intercept(experimental::InterceptorBatchMethods* methods) override
    {
        ...
        if (methods->QueryInterceptionHookPoint(experimental::InterceptionHookPoints::POST_SEND_MESSAGE)) {
            PROFILER_LOG_DEBUG(LOG_CORE, "POST_SEND_MESSAGE method: %s", method);
        } else if (methods->QueryInterceptionHookPoint(experimental::InterceptionHookPoints::POST_RECV_MESSAGE)) {
            PROFILER_LOG_DEBUG(LOG_CORE, "POST_RECV_MESSAGE method: %s", method);
        }
        methods->Proceed();
    }
...
bool ProfilerService::StartService(const std::string& listenUri)
{
    ...
    PROFILER_LOG_INFO(LOG_CORE, "Service started successfully.");
...
{
    if (server_) {
        PROFILER_LOG_INFO(LOG_CORE, "waiting Server...");
        server_->Wait();
        PROFILER_LOG_INFO(LOG_CORE, "Server done!");
    }
}
...
void ProfilerService::StopService()
{
    if (server_) {
        server_->Shutdown();
        PROFILER_LOG_INFO(LOG_CORE, "Server stop done!");
    }
}