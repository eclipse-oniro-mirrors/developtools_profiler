...
protected:
    ...
    void StartDependServerStub()
    {
        ...
        sleep(coverageSleepTime);        // UT 覆盖率版本等待5s
    ...
    std::unique_ptr<IProfilerService::Stub> GetProfilerServiceStub()
    {
        ...
        if (grpcChannel == nullptr) {
            PROFILER_LOG_INFO(LOG_CORE, "Create GRPC channel failed!");
            return nullptr;
        }
    ...
    bool StartActiveProcess(int& cpuActivePid)
    {
        ...
        if (processNum == 0) {
            ...
            if (cpuPlugin == nullptr) {
                PROFILER_LOG_ERROR(LOG_CORE, "test: cpuPlugin is null");
                return false;
            }
    ...
    void StartRequestMemoryProcess(int& requestMemoryPid)
    {
        ...
        if (processNum == 0) {
            ...
            if (requestBuff == nullptr) {
                ...
                char buf[bufSize] = { 0 };
                strerror_r(errno, buf, bufSize);
                PROFILER_LOG_ERROR(LOG_CORE, "StartRequestMemoryProcess: malloc %zu fail, errno(%d:%s)",
    ...
    void IoTest()
    {
        ...
        if (writeFp == nullptr) {
            PROFILER_LOG_ERROR(LOG_CORE, "fopen() error");
            return;
        }
        size_t len = fwrite(const_cast<char*>(str.c_str()), 1, BLOCK_LEN, writeFp);
        if (len < 0) {
            PROFILER_LOG_ERROR(LOG_CORE, "fwrite() error");
            if (fclose(writeFp) != 0) {
        ...
        if (ret == EOF) {
            PROFILER_LOG_ERROR(LOG_CORE, "fflush() error");
            if (fclose(writeFp) != 0) {
                PROFILER_LOG_ERROR(LOG_CORE, "fclose() error");
            }
            return;
...
HWTEST_F(ProfilerServicePerformanceTest, DFX_DFR_Hiprofiler_0010, Function | MediumTest | Level1)
{
    ...
    std::thread keepSessionThread1([&]() {
        while (sendHeart && loopCount < LOOP_COUNT) {
            ++loopCount;
            KeepSessionRequest request;
            request.set_request_id(0);
    ...
    std::thread keepSessionThread2([&]() {
        while (sendHeart2 && loopCount < LOOP_COUNT) {
            ++loopCount;
            KeepSessionRequest request;
            request.set_request_id(0);
