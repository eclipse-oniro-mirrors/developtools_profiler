                                                           ...
                                                           const ProfilerStateRepeaterPtr& stateRepeater)
{
    ...
    auto session = std::make_shared<PluginSession>(pluginConfig, bufferConfig, pluginService_,
                                                   dataRepeater, stateRepeater);
                                                           ...
                                                           const ProfilerStateRepeaterPtr& stateRepeater)
{
    ...
    auto session = std::make_shared<PluginSession>(pluginConfig, pluginService_, dataRepeater, stateRepeater);
    CHECK_NOTNULL(session, nullptr, "allocate plugin session for %s failed!", name.c_str());
    CHECK_TRUE(session->IsAvailable(), nullptr, "config plugin for %s failed!", name.c_str());
    return session;
}
                                                ...
                                                const ProfilerStateRepeaterPtr& stateRepeater)
{
    ...
    for (size_t i = 0; i < pluginConfigs.size(); i++) {
        auto session = CreatePluginSession(pluginConfigs[i], bufferConfigs[i], dataRepeater, stateRepeater);
                                                ...
                                                const ProfilerStateRepeaterPtr& stateRepeater)
{
    std::map<std::string, PluginSessionPtr> tmpPluginSessions;
    for (size_t i = 0; i < pluginConfigs.size(); i++) {
        auto session = CreatePluginSession(pluginConfigs[i], dataRepeater, stateRepeater);
...
bool PluginSessionManager::StartPluginSessions(const std::vector<std::string>& nameList)
{
    ...
    for (auto& name : nameList) {
        ...
        if (!it->second->Start()) {
            PROFILER_LOG_INFO(LOG_CORE, "start session %s FAILED!", it->first.c_str());
            failureCount++;
        }
    }
...
bool PluginSessionManager::StopPluginSessions(const std::vector<std::string>& nameList)
{
    ...
    for (auto& name : nameList) {
        ...
        if (!it->second->Stop()) {
            PROFILER_LOG_INFO(LOG_CORE, "stop session %s FAILED!", it->first.c_str());
            failureCount++;
        }
    }
    return failureCount == 0;
...
bool PluginSessionManager::RefreshPluginSession()
{
    ...
    for (auto& name : pluginNameList_) {
        ...
        if (!it->second->Refresh()) {
            PROFILER_LOG_INFO(LOG_CORE, "refresh data %s FAILED!", it->first.c_str());
            failureCount++;
        }
    }
    return failureCount == 0;
...
bool PluginSessionManager::CheckStatus(const std::vector<std::string>& nameList, PluginSession::State state)
{
    ...
    for (auto& name : nameList) {
        ...
        if (actualState != state) {
            PROFILER_LOG_INFO(LOG_CORE, "plugin %s state is %d not %d!", it->first.c_str(), actualState, state);
            return false;
        }
    }
    return true;
