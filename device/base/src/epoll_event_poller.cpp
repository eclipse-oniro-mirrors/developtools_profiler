...
bool EpollEventPoller::Init()
{
    ...
    eventFdCtx->onReadable = ([this] { this->OnNotify(); });
...
bool EpollEventPoller::Finalize()
{
    ...
    for (auto ctxPtr : contextVec) {
        ...
        UpdateEvent(EPOLL_CTL_DEL, ctxPtr);
    }
...
bool EpollEventPoller::Start()
{
    ...
    if (state_ == STARTED) {
        PROFILER_LOG_INFO(LOG_CORE, "epoll thread has started!");
        return true;
    }
    std::unique_lock<std::mutex> lock(mutex_);
    CHECK_TRUE(state_ == INITIED, false, "Start FAIL %d", state_.load());
    running_ = true;
    pollThread_ = std::thread([this] { this->Run(); });
    state_ = STARTED;
    return true;
}
...
{
    std::unique_lock<std::mutex> lock(stopPollerMutex_);
