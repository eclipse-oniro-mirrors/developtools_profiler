...
int BPFController::SetUpBPF()
{
    ...
    if (config_.maxStackDepth_ > MAX_SIZE) {
        HHLOGE(true, "SetUpBPF maxStackDepth exceeds max size");
        return -1;
    }
...
uint64_t BPFController::GetSymOffset(const std::string &path, const std::string &symbol)
{
    ...
    CHECK_NOTNULL(symData, 0, "symData is NULL!");
    ...
    CHECK_NOTNULL(strData, 0, "strData is NULL!");
